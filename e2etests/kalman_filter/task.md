...
测试文件已给定为 `kalman_test.cj`，**不要做修改**；必须逐字节复制到项目 `src/kalman_test.cj`，使用仓颉单元测试框架执行。
...
1. **C 库编译成功**：Windows 交付同时提供链接名 `libkalman_filter` 与运行时加载名 `kalman_filter.dll`（可复制同一 DLL 作为两个名称）；Linux/macOS 依平台动态库命名约定交付
