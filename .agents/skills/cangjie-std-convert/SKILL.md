...
description: "仓颉标准库 std.convert 常用类型转换操作。当需要将字符串解析为基础类型（整数、浮点、布尔等）、整数进制转换，或修复 HumanEval/算法题中的 Int64.parse(s) ?? 0、Float64.parse(...).get、parse/tryParse 混用等编译错误时，应使用此 Skill。涉及 Parsable 和 RadixConvertible 接口。"
---
...
main() {
    ...
    let userInput = "999"
    let safe = Int64.tryParse(userInput) ?? 0  // 安全解析 + 默认值
...
> **注意**：`parse()` 返回目标类型本身，不是 `Option<T>`；不要写 `Int64.parse(s) ?? 0`，也不要对 `Int64.parse()` / `Float64.parse()` 的结果调用 `.get()` 或 `.getOrThrow()`。需要安全解析和默认值时使用 `tryParse() ?? fallback`。浮点数解析暂不支持二进制和八进制格式。Rune 解析需要带引号，如 `"'a'"`。
