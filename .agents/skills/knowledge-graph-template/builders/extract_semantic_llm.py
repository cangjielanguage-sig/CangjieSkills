...
- INFERRED: reasonable inference. In this domain, ALWAYS emit these inferred edges when the evidence is there:
  ...
  * Cross-ecosystem analogies (e.g. Android `RecyclerView` / `FlatList` / `SwipeRefreshLayout`): add `semantically_similar_to` (INFERRED, confidence 0.75-0.9) between the analogy phrase and the concrete HarmonyOS / ArkUI counterparts (`List`, `LazyForEach`, `Refresh`, `Scroll`) when both appear in the same doc chunk.
