...
// 此处代码可添加在依赖项定义中
class MediaDataHandler <: MediaAssetDataHandler<Array<Byte>> {
    public func onDataPrepared(data: Array<Byte>, map: HashMap<String, String>): Unit {
        AppLog.info("success")
    }
}
...
let requestId = MediaAssetManager.requestImageData(ctx, asset, requestOptions, handler)
```