...
`cangjie-skill-evolution` 用来把日常开发 Skill 时产生的失败记录、成功记录、rollout、工具调用、视觉观察、用户反馈和测试结果，转化为可执行、可验证的 Skill 改进。
...
- 只修复 `skill-lint` 报错或 warning。此时应使用 `cangjie-skill-lint-fix`。
...
```text
请使用 cangjie-skill-evolution 分析 .agents/skills/my-skill。
执行分析评审模式，不修改文件。
...
```text
请使用 cangjie-skill-evolution 实际改进 .agents/skills/my-skill。
证据目录是 .agents/skills/my-skill/evidence/evolution/2026-06-05-basic-behavior。
...
```text
请使用 cangjie-skill-evolution 治理 .agents/skills 技能库。
...
```text
请使用 cangjie-skill-evolution 实际改进 .agents/skills/json-report。
