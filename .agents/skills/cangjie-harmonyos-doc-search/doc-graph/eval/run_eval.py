...
graph 在内存中搜索（加载一次，复用 session），card 直接调 search_cards
（跳过 understanding，只测纯搜索），fusion 组合两者。

用法:
    python eval/run_eval.py                        # 全量评测
    python eval/run_eval.py --limit 50             # 只测前50条
    python eval/run_eval.py --output eval/report.md

输出: eval/comparison_report.md + 控制台摘要
"""
...
EVAL_DIR = SKILL_DIR / "doc-graph" / "eval"
