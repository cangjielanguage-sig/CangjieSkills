...
`main(args): Int64` 用非零值报告失败；测试退出码时直接运行构建产物，因为 Windows 上 cjpm 1.1.3 不透传该状态。

## 核心指导

需要向调用脚本报告失败时，让 `main(args)` 返回 `Int64`：`0` 表示成功，非零值区分用法错误或业务错误；诊断写入 stderr。Windows x64 cjnative 1.1.3 实测中，`cjpm run` 自身仍返回 0，不会透传程序的非零状态，因此自动测试必须构建后直接启动生成的可执行文件。
