...
source ../../cangjie-harmonyos-doc-search-maintenance/scripts/load_env.sh

OPENAI_BASE_URL="https://api.modelarts-maas.com/openai/v1" \
OPENAI_MODEL="deepseek-v3.2" \
OPENAI_TEMPERATURE="0" \
python ../../cangjie-harmonyos-doc-search-maintenance/scripts/run_maintenance.py \
...
本 Skill 与 `doc-graph`（下称 KG）**平权并存**，按 query 类型分发。两条引擎共享同一套文档源（harmonyos-6.0.2-15k / lang-features / std / stdx / tools），agent 侧融合结果即可。两者均已归入统一入口 `cangjie-harmonyos-doc-search`，详见顶层 SKILL.md。
...
| 功能实现类（"做一个 X"）| V3 默认 + KG 组合 | V3 `--mode task` + KG `query_graph` |
| 跨概念组合（"带下拉刷新的网络列表页"）| KG | CLI `search` + `neighbors` |
| 语义模糊描述（"卡顿"/"响应式失效"）| KG | CLI `search`（语义边） |
| 跨生态类比（"鸿蒙版 RecyclerView"）| KG | CLI `search` + `god-nodes` |
| "A 和 B 怎么配合" / 依赖链 | KG | CLI `path` + `neighbors` |
| "这个领域的核心 API" | KG | CLI `god-nodes` |
| 架构鸟瞰 / 社区洞察 | KG | CLI `community` |

### V3 调用（精确检索）

```bash
python <CangjieSkills>/.agents/skills/cangjie-harmonyos-doc-search/doc-card/search_v3.py "<query>" --json --limit 5
```
...
### doc-graph KG 调用（语义 / 组合 / 架构）

通过 CLI 调用（位于 `doc-graph/cli.py`）：

```bash
python <CangjieSkills>/.agents/skills/cangjie-harmonyos-doc-search/doc-graph/cli.py search "关键词" --graph doc -b -k 5
python <CangjieSkills>/.agents/skills/cangjie-harmonyos-doc-search/doc-graph/cli.py neighbors "List"
python <CangjieSkills>/.agents/skills/cangjie-harmonyos-doc-search/doc-graph/cli.py path "UIAbility" "WindowStage"
python <CangjieSkills>/.agents/skills/cangjie-harmonyos-doc-search/doc-graph/cli.py god-nodes --top-n 10
```
...
1. **精确命中优先**：V3 返回的 `paths`（匹配 `HIGH_VALUE_API_MAP` 的）排在 doc-graph KG 之前
2. **按 `source_file` / `source_paths` 去重**：同一文档不重复
3. **doc-graph KG 补充邻域**：V3 没覆盖的相关概念从 KG `neighbors` 拉进来作为"扩展阅读"
4. **矛盾时以 V3 为准**：API 名、签名、参数类型这类事实性信息以 V3 卡片为单一事实源，KG 只提供关联线索
