...
description: "仓颉标准库 ArrayList 类型详细指南。当需要了解 ArrayList 的构造、增删改查、容量管理、切片、排序、迭代、与 Array 互转等操作，或修复 HumanEval/算法题中的 ArrayList.append、Array 与 ArrayList 互转、动态收集后返回 Array 等编译错误时，应使用此 Skill。"
---
...
> **HumanEval P0 自查**：`ArrayList<T>` 追加元素用 `.add(value)`，没有 `.append()`；元素数量用 `.size` 属性，不写 `.length` / `.size()`。函数签名要求返回 `Array<T>` 时，内部可用 `ArrayList<T>` 动态收集，最后 `return list.toArray()`。
...
不要写 `list.append(x)`；仓颉 `ArrayList` 的追加方法是 `add`。
...
HumanEval/新代码中不要使用以下废弃方法；优先 `import std.sort.*` 后调用全局 `sort(list)`。
