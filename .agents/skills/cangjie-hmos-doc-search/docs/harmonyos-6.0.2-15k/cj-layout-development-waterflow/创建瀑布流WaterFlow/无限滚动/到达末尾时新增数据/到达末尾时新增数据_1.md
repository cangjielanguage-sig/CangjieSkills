...
瀑布流常用于无限滚动的信息流。可以在瀑布流组件到达末尾位置时触发的[onReachEnd](../../../../cj-scroll-swipe-waterflow/.overview.md)事件回调中对[LazyForEach](../../../../cj-state-rendering-lazyforeach/.overview.md)增加新数据，并将footer做成正在加载新数据的样式（使用[LoadingProgress](../../../../cj-information-display-loadingprogress/.overview.md)组件）。
