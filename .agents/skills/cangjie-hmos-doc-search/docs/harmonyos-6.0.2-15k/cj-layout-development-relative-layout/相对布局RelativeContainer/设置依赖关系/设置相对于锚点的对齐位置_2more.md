...
设置了锚点之后，可以通过[alignRules](../../../cj-universal-attribute-location/.overview.md)属性设置相对于锚点的对齐位置。
...
子组件经过相对位置对齐后，位置可能还不是目标位置，开发者可根据需要进行额外偏移设置额外偏移（offset）。当使用offset调整位置的组件作为锚点时，对齐位置为设置offset之前的位置。建议使用[bias](../../../cj-universal-attribute-location/.overview.md)来设置额外偏移。
