...
_CONCEPT_GROUPS = {
    "list": {
        "triggers": ["list", "列表", "长列表", "cj-scroll-swipe-list"],
        "terms": ["list", "lazyforeach", "cj-scroll-swipe-list"],
    },
    "refresh": {
        "triggers": ["refresh", "刷新", "下拉刷新", "cj-scroll-swipe-refresh"],
        "terms": ["refresh", "cj-scroll-swipe-refresh"],
    },
    "http": {
        "triggers": ["http", "httprequest", "网络", "网络请求", "networkkit"],
        "terms": ["httprequest", "http", "networkkit", "cj-apis-net-http"],
    },
    "state": {
        "triggers": ["state", "状态", "刷新ui", "ui刷新"],
        "terms": ["state", "@state", "cj-ui-state"],
    },
    "webview": {
        "triggers": ["webview", "web view", "arkweb", "双向传数据", "javascriptproxy"],
        "terms": ["webview", "webviewcontroller", "registerjavascriptproxy", "arkweb"],
    },
}


def _select_diverse_start_nodes(G: nx.Graph, query: str, scored: list[tuple[float, str]], max_nodes: int = 6) -> list[str]:
    lowered = query.lower()
    selected: list[str] = []
    seen: set[str] = set()

    for group in _CONCEPT_GROUPS.values():
        if not any(trigger in lowered for trigger in group["triggers"]):
            continue
        group_scores = score_nodes(G, group["terms"])
        for _, nid in group_scores:
            if nid not in seen:
                selected.append(nid)
                seen.add(nid)
                break
...
def traverse(
    ...
    start_nodes = _select_diverse_start_nodes(G, query, scored)
