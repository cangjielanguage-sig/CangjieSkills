...
def _import_cangjie(node, source: bytes, file_nid: str, stem: str, edges: list, str_path: str) -> None:
    for child in node.children:
...
_CANGJIE_CONFIG = LanguageConfig(
    ...
    call_function_field="",
    call_accessor_node_types=frozenset({"fieldAccess"}),
    call_accessor_field="",
    name_field="name",
...
def _extract_generic(path: Path, config: LanguageConfig) -> dict:
    ...
    def walk_calls(node, caller_nid: str) -> None:
        ...
        if node.type in config.call_types:
            ...
            elif config.ts_module == "tree_sitter_cangjie":
                # Cangjie: postfixExpression with callSuffix child = function call
                has_call = any(c.type == "callSuffix" for c in node.children)
                if has_call:
                    # Find the deepest postfixExpression, then extract callee from fieldAccess
                    deepest = node
                    for child in node.children:
                        if child.type == "postfixExpression":
                            deepest = child
                            break
                    # Check for fieldAccess first (method call like obj.method())
                    field_access_node = None
                    atomic_var_node = None
                    for child in deepest.children:
                        if child.type == "fieldAccess":
                            field_access_node = child
                        elif child.type == "atomicVariable":
                            atomic_var_node = child
                    if field_access_node:
                        for sub in field_access_node.children:
                            if sub.type == "atomicVariable":
                                for s in sub.children:
                                    if s.type == "varBindingPattern":
                                        callee_name = _read_text(s, source)
                                        break
                    elif atomic_var_node:
                        for sub in atomic_var_node.children:
                            if sub.type == "varBindingPattern":
                                callee_name = _read_text(sub, source)
                                break
            else:
...
def extract_cangjie(path: Path) -> dict:
    """Extract structs, classes, enums, functions, and imports from a .cj file."""
    return _extract_generic(path, _CANGJIE_CONFIG)
