...
MAINTENANCE_DIR = Path(__file__).resolve().parent.parent
...
KEYWORDS_PATH = EVAL_DIR / "keywords" / "keywords_harmonyos.json"
DATASET_PATH = EVAL_DIR / "datasets" / "eval_queries_harmonyos.jsonl"
DEFAULT_REPORT = EVAL_DIR / "reports" / "comparison_report.md"
...
def main():
    ...
    parser.add_argument("--source", default=None,
                        help="按文档源选择评测集 (harmonyos/std/stdx/kernel/tools/all)")
    parser.add_argument("--graph-path", default=None,
                        help="自定义 graph.json 路径 (如 data/doc/graph_cj.json)")
    parser.add_argument("--output", default=str(DEFAULT_REPORT))
    args = parser.parse_args()

    # --source 快捷选择: 自动定位 datasets/eval_queries_{source}.jsonl
    if args.source and not args.dataset:
        if args.source == "all":
            args.dataset = str(EVAL_DIR / "datasets" / "eval_queries_cangjie.jsonl")
        else:
            candidate = EVAL_DIR / "datasets" / f"eval_queries_{args.source}.jsonl"
            if candidate.exists():
                args.dataset = str(candidate)
            else:
                print(f"ERROR: {candidate} 不存在")
                sys.exit(1)
    ...
    graph_path = args.graph_path if args.graph_path else str(GRAPH_PATH)
    graph_data_dir = str(Path(graph_path).parent) if args.graph_path else str(GRAPH_DATA_DIR)
    print(f"Loading graph: {graph_path}")
    session = create_session(graph_dir=graph_data_dir)
    session.doc_engine = None
    session.load_doc_graph(graph_path)
