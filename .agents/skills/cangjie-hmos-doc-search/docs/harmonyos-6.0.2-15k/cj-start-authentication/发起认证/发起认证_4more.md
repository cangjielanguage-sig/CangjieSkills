...
- 统一固定的UI组件样式，便于用户识别。
    ...
    认证控件的样式如图所示，通过[WidgetParam](../../cj-apis-user_auth/.overview.md)配置对应参数。
...
2. 指定用户认证相关参数[AuthParam](../../cj-apis-user_auth/.overview.md)（包括挑战值、认证类型[UserAuthType](../../cj-apis-user_auth/.overview.md)列表和认证等级[AuthTrustLevel](../../cj-apis-user_auth/.overview.md)）、配置认证控件界面[WidgetParam](../../cj-apis-user_auth/.overview.md)，调用[getUserAuthInstance](../../cj-apis-user_auth/.overview.md)获取认证对象。
3. 调用[UserAuthInstance.on](../../cj-apis-user_auth/.overview.md)接口订阅认证结果。
4. 调用[UserAuthInstance.start](../../cj-apis-user_auth/.overview.md)接口发起认证，通过[UserAuthInstance.on](../../cj-apis-user_auth/.overview.md)的回调返回认证结果[UserAuthResult](../../cj-apis-user_auth/.overview.md)。