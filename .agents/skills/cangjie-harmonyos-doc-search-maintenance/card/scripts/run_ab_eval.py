...
本模块是 card 分区维护流水线中的"语义能力评测"组件，用于对同一评测集
分别跑 V3（规则索引）、graphify（知识图谱）、fusion（两者合并）三种
搜索引擎，比较 recall_at_k、paraphrase 方差和 composition 召回率。

被 run_maintenance.py 的 fusion_ab_gate 和
run_semantic_capability_gate.py 调用；产物为 JSON 报告，
供门禁判定 fusion 是否优于单引擎。
"""
...
def path_hit(paths: list[str], acceptable_paths: list[str]) -> bool:
    ...
    因为 V3 和 graphify 的路径粒度可能不同（一个返回目录，另一个返回文件）。"""
...
def cluster_hit(paths: list[str], clusters: list[str]) -> dict[str, bool]:
    """概念聚类命中检测：判定搜索结果是否覆盖了指定的概念类。
    aliases 映射将同一概念的不同表述（如 state/appstorage/localstorage）
    映射到统一的 cluster 名，用于 composition 评测集的多维度召回评估。"""
...
def evaluate_rows(rows: list[dict[str, Any]], engine_name: str, run_one, limit: int) -> dict[str, Any]:
    ...
    参数 run_one 为单条查询的搜索函数，接收 query 字符串，返回 {paths, raw}。
    """
    cases = []
    ...
    # paraphrase_variance：同一意图不同表述命中的总体方差（population variance），
    # 衡量引擎对语义等价查询的稳定性，0=完全一致
    paraphrase_variance = {
