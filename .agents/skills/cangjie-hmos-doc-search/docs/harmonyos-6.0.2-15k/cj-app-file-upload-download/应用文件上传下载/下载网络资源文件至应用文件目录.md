...
开发者可以使用上传下载模块（[ohos.request](../../cj-apis-request-agent/.overview.md)）的下载接口将网络资源文件下载到应用文件目录。对已下载的网络资源文件，开发者可以使用基础文件IO接口（[ohos.file_fs](../../cj-apis-file_fs/.overview.md)）对其进行访问，使用方式与[应用文件访问](../../cj-app-file-access/.overview.md)一致。文件下载过程使用系统服务代理完成，支持用户设置自定义代理地址。
