...
本文面向第一次接触 `cangjie-hmos-doc-search` 的维护者和使用者，说明卡片推荐方案是什么、为什么这样做、如何构建、如何查询，以及常见术语的含义。
...
维护阶段由 `cangjie-hmos-doc-search-maintenance` 负责。构建脚本读取 `cangjie-hmos-doc-search/docs/` 下的文档，生成结构化索引并写入 `cangjie-hmos-doc-search/doc-card/index/`。

查询阶段由 `cangjie-hmos-doc-search` 负责。用户态只读本地索引，不调用 LLM，不要求用户配置 `url/key/model`。
...
```text
cangjie-hmos-doc-search/docs/<source_path>
```
...
```bash
python .agents/skills/cangjie-hmos-doc-search-maintenance/builder/build_index_v3.py \
  --mode rule
```
...
```bash
python .agents/skills/cangjie-hmos-doc-search/doc-card/search_v3.py \
  "List 下拉刷新示例" \
  --mode example \
  --json \
  --limit 5
...
```bash
python .agents/skills/cangjie-hmos-doc-search/unified_search.py \
  "List 下拉刷新示例" \
  --engine card \
  --json \
  --limit 5
...
```bash
ls -lh .agents/skills/cangjie-hmos-doc-search/doc-card/index/docs.jsonl
ls -lh .agents/skills/cangjie-hmos-doc-search/doc-card/index/search.db
```
...
```bash
python .agents/skills/cangjie-hmos-doc-search-maintenance/builder/build_index_v3.py --help
