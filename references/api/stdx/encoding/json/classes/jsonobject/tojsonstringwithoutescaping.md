<!-- cj-doc kind="api-member" level="6" id="stdx.encoding.json.class.jsonobject.tojsonstringwithoutescaping" parent="stdx.encoding.json.class.jsonobject" -->
# JsonObject.toJsonStringWithoutEscaping
...
将 JsonObject 转换为 JSON 格式的 (带有空格换行符) 字符串，不对 html 特殊字符 `&` 转义。
...
将 JsonObject 转换为 JSON 格式的字符串。该函数将指定初始的缩进深度、第一个括号后是否换行以及缩进字符串，不对 html 特殊字符 `&` 转义。
