...
description: "维护 cangjie-harmonyos-doc-search 的文档更新、索引/卡片重建、图谱构建、LLM 离线增强、评测集健康检查、严格 blind 轮换和发布门禁流程。适用于文档路径或内容变化后重新生成检索索引、图谱数据和评测素材并判断 pass/gray_release/blocked。"
...
`cangjie-harmonyos-doc-search` 是用户面向的搜索运行时（仅搜索，不构建），包含：
...
- card → `cangjie-harmonyos-doc-search/doc-card/index/`
- graph → `cangjie-harmonyos-doc-search/doc-graph/data/`
...
PYTHONDONTWRITEBYTECODE=1 python graph/builder/build_cli.py build \
  ...
  --graph-dir cangjie-harmonyos-doc-search/doc-graph/data
...
PYTHONDONTWRITEBYTECODE=1 python fusion/scripts/run_ab_eval.py \
  ...
  --index-dir cangjie-harmonyos-doc-search/doc-card/index \
  --graph-dir cangjie-harmonyos-doc-search/doc-graph/data \
