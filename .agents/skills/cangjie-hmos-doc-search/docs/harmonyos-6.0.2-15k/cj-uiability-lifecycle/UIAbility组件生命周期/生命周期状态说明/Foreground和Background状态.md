...
Foreground和Background状态分别在[UIAbility](../../../cj-apis-ability/.overview.md)实例切换至前台和切换至后台时触发，对应于[onForeground()](../../../cj-apis-ability/.overview.md)回调和[onBackground()](../../../cj-apis-ability/.overview.md)回调。
...
当应用的UIAbility实例已创建，且UIAbility配置为[singleton](cj-uiability-launch-type.md#singleton启动模式)启动模式时，再次调用[startAbility()](../../../cj-apis-ability/.overview.md)方法启动该UIAbility实例时，只会进入该UIAbility的[onNewWant()](../../../cj-apis-ability/.overview.md)回调，不会进入其[onCreate()](../../../cj-apis-ability/.overview.md)和[onWindowStageCreate()](../../../cj-apis-ability/.overview.md)生命周期回调。应用可以在该回调中更新要加载的资源和数据等，用于后续的UI展示。
