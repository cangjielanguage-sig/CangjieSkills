...
|参数名|类型|必填|默认值|说明|
|:---|:---|:---|:---|:---|
|\`type\`|[AudioRendererCallbackType](#enum-audiorenderercallbacktype)|是|-|监听事件，固定为：'AR_OUTPUT_DEVICE_CHANGE_WITH_INFO'。|
|callback|[Callback1Argument](../../../cj-apis-base/.overview.md)\<[AudioStreamDeviceChangeInfo](#class-audiostreamdevicechangeinfo)>|是|-|回调函数，返回当前音频流的输出设备描述信息及变化原因。|
...
// 此处代码可添加在依赖项定义中
class outputDeviceChangeWithInfoCallback <: Callback1Argument<AudioStreamDeviceChangeInfo> {
    public func invoke(arg: AudioStreamDeviceChangeInfo) {
        AppLog.info("outputDeviceChangeWithInfoCallback:${arg.changeReason}")
    }
}
...
try {
    ...
    try {
        ...
        let cb = outputDeviceChangeWithInfoCallback()
        render.on(AudioRendererCallbackType.AR_OUTPUT_DEVICE_CHANGE_WITH_INFO, cb)
        Hilog.error(0, "test_AR_OutputDevicewithInfoCallback", "outputDeviceChangeWithInfoCallback")
    } catch (e: BusinessException) {
        Hilog.error(0, "AudioRendererCallbackType.AR_OUTPUT_DEVICE_CHANGE_WITH_INFO", "errCode: ${e.code}, errMessage: ${e.message}")
    }
} catch (e: BusinessException) {
