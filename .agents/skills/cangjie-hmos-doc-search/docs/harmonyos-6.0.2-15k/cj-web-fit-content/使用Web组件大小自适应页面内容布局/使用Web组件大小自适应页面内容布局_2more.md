...
1. 建议配置[过滚动模式](../../cj-web-web/.overview.md)为关闭状态。当过滚动模式开启时，当用户在Web界面上滑动到边缘时，Web会通过弹性动画弹回界面，会与Scroll组件的回弹相互冲突，影响体验。
2. [键盘避让](../../cj-web-web/.overview.md)属性配置为RESIZE_CONTENT时，该避让模式不生效。
3. 不支持对页面进行缩放。
