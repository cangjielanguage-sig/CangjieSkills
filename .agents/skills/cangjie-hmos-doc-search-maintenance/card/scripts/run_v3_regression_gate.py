...
本脚本专注于结构化索引自身的召回质量验证，不涉及 graph/fusion 语义能力。
与 run_semantic_capability_gate.py（fusion 能力门禁）拆分，避免把结构化召回
与语义/融合能力绑在同一阈值上。

门禁判定：overall recall@5 低于 --min-recall-at-5 阈值时 exit 1。
默认阈值 0 表示仅产出报告不强制通过。
"""
...
ROOT = SKILLS_DIR / "cangjie-hmos-doc-search"
