...
**功能：** 根据不同的策略模式，请求动态照片对象。动态照片对象可用于请求动态照片的资源数据。
...
|dataHandler|[MediaAssetDataHandler](#class-mediaassetdatahandler)\<[MovingPhoto](#class-movingphoto)>|是|-|媒体资源处理器，当所请求的图片资源准备完成时会触发回调。|
...
// 此处代码可添加在依赖项定义中
class MovingPhotoHandler <: MediaAssetDataHandler<MovingPhoto> {
    public func onDataPrepared(data: MovingPhoto, map: HashMap<String, String>): Unit {
        let imageFileUri = "file://com.example.xxx/data/storage/el2/base/haps/entry/files/ImageFile.jpg"
        let videoFileUri = "file://com.example.xxx/data/storage/el2/base/haps/entry/files/VideoFile.mp4"
        data.requestContent(imageFileUri, videoFileUri)
        AppLog.info("moving photo contents retrieved successfully")
    }
}
...
let handler = MovingPhotoHandler()
let fetchResult = phAccessHelper.getAssets(fetchOptions)
let asset = fetchResult.getFirstObject()
let requestId = MediaAssetManager.requestMovingPhoto(ctx, asset, requestOptions, handler)
```