<!-- cj-doc kind="api-member" level="6" id="std.math.numeric.struct.bigint.tryparse" parent="std.math.numeric.struct.bigint" -->
# BigInt.tryParse
...
- ?BigInt - 解析出的 BigInt 结构体，解析失败则返回 `None`。
...
尝试根据指定进制将字符串解析成一个 BigInt 结构体，支持 2 进制到 36 进制。
...
- ?BigInt - 解析出的 BigInt 结构体，解析失败时返回 `None`。
