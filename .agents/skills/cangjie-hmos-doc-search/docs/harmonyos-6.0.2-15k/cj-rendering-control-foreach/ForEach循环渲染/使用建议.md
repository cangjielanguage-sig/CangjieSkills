...
- ForEach在下列容器组件 [List](../../cj-scroll-swipe-list/.overview.md)、[Grid](../../cj-scroll-swipe-grid/.overview.md)、[Swiper](../../cj-scroll-swipe-swiper/.overview.md)以及[WaterFlow](../../cj-scroll-swipe-waterflow/.overview.md)内使用的时候，不要与LazyForEach 混用。 以List为例，同时包含ForEach、LazyForEach的情形是不推荐的。
