...
// 此处代码可添加在依赖项定义中
class wdata_Callback <: Callback1ArgumentWithReturn<Array<Byte>, AudioDataCallbackResult> {
    public func invoke(arg: Array<Byte>): AudioDataCallbackResult {
        AppLog.info("wdata_Callback:${arg.size}")
...
try {
    ...
    try {
        ...
        let cb = wdata_Callback()
        render.on(AudioRendererCallbackType.AR_WRITE_DATA, cb)
        sleep(Duration.second * 1)
        render.start()
    } catch (e: BusinessException) {
        Hilog.error(0, "AudioRendererCallbackType.AR_WRITE_DATA", "errCode: ${e.code}, errMessage: ${e.message}")
    }
} catch (e: BusinessException) {
