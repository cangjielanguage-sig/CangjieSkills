...
- `Int64` 的公开极值属性是 `Int64.Max` 和 `Int64.Min`；不要从其它语言迁移 `Int64.MaxValue`、`Int64.MinValue` 等名称。其它整数类型的极值也必须先在对应类型契约中确认。
- 从已知非空集合求最小值或最大值时，优先用第一个有效元素初始化并扫描剩余元素，不必引入极值哨兵。
- 输入可能为空时，根据函数契约使用 `Option`、显式初始化标记或空输入分支；只有算法确实需要完整类型边界时才使用 `Int64.Max`/`Int64.Min`。
...
### 5.2 赋值语境中的字面量转换
- 该转换只适用于形如 `left = right` 的赋值，包括带显式类型的 `let`/`var` 初始化和后续重新赋值：左侧为 `Byte`（`UInt8` 别名）时，右侧 ASCII 单字符字符串字面量可转为 `Byte`；左侧为 `Rune` 时，右侧单字符字符串字面量可转为 `Rune`
- 它不适用于 `==`/`!=` 等比较、函数实参、返回表达式、算术表达式或 `String(...)` 构造；这些位置必须使用匹配静态类型的 `b'x'`、`r'x'` 或经文档确认的显式转换
...
- `for (b in s)` 中的 `b` 是 `UInt8`：字节比较使用 `b'x'`，不能与普通字符串/Rune 字面量比较，也不能把 `String(b)` 当成字符构造。需要字符语义时改用 `s.runes()`，元素类型为 `Rune`，并用 `r'x'` 比较
