...
description: "仓颉标准库 String 类型详细指南。当需要了解 String 的构造、搜索、替换、分割、拼接、裁剪、大小写转换、编码处理、下标访问、迭代等操作，或修复 HumanEval/算法题中的 String.toLower/toUpper、String.fromRune、String(Rune)、String.fromRuneArray、UInt8==Rune、length/size 等字符串编译错误时，应使用此 Skill。"
---
...
> **HumanEval P0 自查**：普通 `String` 没有 `.toLower()` / `.toUpper()` / `.length` / `String.fromRune()` / `String.fromRuneArray()` / `String(rune)`。ASCII 大小写用 `toAsciiLower()` / `toAsciiUpper()`；单个 Rune 转字符串用 `String([r])`；`ArrayList<Rune>` 先 `.toArray()` 再 `String(runeList.toArray())`。
...
let one = String([r'!'])        // 单个 Rune 转 String；不要写 String(r)

// 从 ArrayList<Rune> 构造时先转 Array<Rune>（需 import std.collection.*）
let runeList = ArrayList<Rune>(runes)
let s7 = String(runeList.toArray())
...
> **HumanEval 常见坑**：不要写 `s.toLower()` / `s.toUpper()`；普通 ASCII 字符串用 `s.toAsciiLower()` / `s.toAsciiUpper()`。
