...
开发者可以通过接口[getDefaultInputMethod](../cj-apis-inputmethod/.overview.md)查询系统配置默认输入法，判断当前应用是否为默认输入法，若不是，则不支持调用此接口。
...
当调用[输入法框架](../cj-apis-inputmethod/.overview.md)中的任何接口都有可能由于依赖输入法管理服务，而服务找不到时发生此异常。
