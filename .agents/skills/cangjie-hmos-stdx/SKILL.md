...
```bash
python .agents/skills/cangjie-hmos-stdx/scripts/fetch_stdx.py --platform all
```

也可以只下载单个平台：

```bash
python .agents/skills/cangjie-hmos-stdx/scripts/fetch_stdx.py --platform x86_64
python .agents/skills/cangjie-hmos-stdx/scripts/fetch_stdx.py --platform aarch64
```
...
2. 检查对应平台 zip 包是否存在；缺失时先运行 `python .agents/skills/cangjie-hmos-stdx/scripts/fetch_stdx.py --platform all`
...
API 详情请使用 cangjie-hmos-doc-search 或 cangjie_stdx Skill 检索

## 排错

1. 检查 `cangjie-stdx-ohos-*.zip` 是否存在 → 缺失则运行 `python .agents/skills/cangjie-hmos-stdx/scripts/fetch_stdx.py --platform all`
