...
public static func on(eventType: ObserverEventType,
    callback: Callback1Argument<Array<SignalInformation>>, options!: ?ObserverOptions = None): Unit
```
...
let observerOptions = ObserverOptions(slotId: 1)
...
TelephonyObserver.on(ObserverEventType.SignalInfoChange, callback2, options: observerOptions)
```