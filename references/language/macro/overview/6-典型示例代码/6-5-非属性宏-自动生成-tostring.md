<!-- cj-doc kind="guide-leaf" level="6" id="language.macro.overview.6-典型示例代码.6-5-非属性宏-自动生成-tostring" parent="language.macro.overview.6-典型示例代码" -->
...
调用处（`src/main.cj`）：
```cangjie cjtest=syntax id=syntax-1373011811-2 form=unit
import macros.*
...
```toml cjtest=project id=language.macro-auto-to-string file=cjpm.toml command=run timeout=120s
[package]
cjc-version = "1.0.5"
name = "auto_to_string"
version = "0.1.0"
...
```toml cjtest=file project=language.macro-auto-to-string file=macros/cjpm.toml
[package]
cjc-version = "1.0.5"
...
```cangjie cjtest=file project=language.macro-auto-to-string file=macros/src/auto_to_string.cj
...
```cangjie cjtest=file project=language.macro-auto-to-string file=src/main.cj
...
```text cjtest=expect for=language.macro-auto-to-string stream=stdout match=exact
