...
将本地页面文件放在应用的rawfile目录下，开发者可以在Web组件创建的时候指定默认加载的本地页面 ，并且加载完成后可通过调用[loadUrl()](../../../cj-apis-webview/.overview.md)接口变更当前Web组件的页面。
...
1. 通过构造的单例对象GlobalContext获取沙箱路径，需要开启应用中文件系统的访问[fileAccess](../../../cj-web-web/.overview.md)权限。
...
2. 修改main_ability.cj。
    ...
    以filesDir为例，获取沙箱路径。若想获取其他路径，请参见[应用文件路径](../../../cj-application-context-stage/.overview.md)。
