...
|dataHandler|[MediaAssetDataHandler](#class-mediaassetdatahandler)\<[ImageSource](../../../cj-apis-image/.overview.md)>|是|-|媒体资源处理器，当所请求的图片资源准备完成时会触发回调。|
...
// 此处代码可添加在依赖项定义中
class MediaHandler <: MediaAssetDataHandler<ImageSource> {
    public func onDataPrepared(data: ImageSource, map: HashMap<String, String>): Unit {
        AppLog.info("success")
    }
}
...
let requestId = MediaAssetManager.requestImage(ctx, asset, requestOptions, handler)
```