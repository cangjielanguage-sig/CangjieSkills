...
系统提供了4种预配置类型（[PreconfigType](../../cj-apis-multimedia-camera/.overview.md)），分别为PRECONFIG_720P、PRECONFIG_1080P、PRECONFIG_4K、PRECONFIG_HIGH_QUALITY。以及3种画幅比例规格（[PreconfigRatio](../../cj-apis-multimedia-camera/.overview.md)），1:1画幅（PRECONFIG_RATIO_1_1）、4:3画幅（PRECONFIG_RATIO_4_3）、16:9画幅（PRECONFIG_RATIO_16_9）。

> **注意：**
>
> 由于不同的设备所支持的能力不同。使用相机预配置（preconfig）功能时，需要先调用[canPreconfig](../../cj-apis-multimedia-camera/.overview.md)检查对应的PreconfigType和PreconfigRatio的组合在当前设备上是否支持。
