...
本流程服务于 `cangjie-hmos-doc-search` 的持续维护。目标是让文档路径或内容变化后，可以可重复地完成索引重建、候选素材生成、评测健康检查和发布判断。
...
- `../cangjie-hmos-doc-search-maintenance/scripts/validate_eval_set.py`：输入评测集、索引和 manifest，输出 `eval-health.json`。
...
`scripts/run_maintenance.py` 是历史 rule/rule+llm/OpenViking AB 流程入口，可用于需要同时沉淀旧 baselines/run-history 的场景。当前发布判断优先使用 `cangjie-hmos-doc-search-maintenance/scripts/run_release_eval.py`。
