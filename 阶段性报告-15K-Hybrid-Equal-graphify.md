...
- `knowledge-graph-template/docs/harmonyos-6.0.2-15k` symlink 指向 `cangjie-hmos-doc-search/harmonyos-6.0.2-15k`。

### V3 索引

- 已跑通 **`build_index_v3.py --mode rule`**，产物在 `cangjie-hmos-doc-search/index/`（约 docs 14824、apis 4635 等，见 manifest）。
- **`rule+llm` 全量**：已用 `CANGJIE_LLM_API_FILE` + 修复后的 `load_env.sh`（避免占位 `OPENAI_API_KEY` 导致 **401**）完成构建并写入默认 **`index/`**；`index/manifest.json` 为 **`generation_mode: rule+llm`**。
  ...
  - 曾有 7 条 `std/sync` 原子类型相关 api/doc 稳定触发 ModelArts **HTTP 403**；已通过 `cangjie-hmos-doc-search-maintenance/scripts/repair_llm_failure_cache.py` 生成本地 fallback cache，并以 `--max-llm-publish-failures 0` 严格重跑成功。fallback 条目带 `llm-fallback-cache` tag 与 `needs_review=true`，后续可人工复核或用稳定 LLM 再补。
- **构建结束后自动收尾**：已跑 **`cangjie-hmos-doc-search-maintenance/scripts/run_after_rule_llm_build.sh`**（等待进程结束 → `sync_v3_to_graph` → `--validate` → `run_ab_eval`），写出 **`records/v3_seeds_15k_post_llm.json`** 与 **`records/ab-15k-after-rule-llm.json`**；日志 **`/tmp/after_rule_llm_build_latest.log`**。
...
- 脚本：`cangjie-hmos-doc-search-maintenance/scripts/regenerate_15k_independent_evals.py`
...
见 `cangjie-hmos-doc-search-maintenance/records/ab-15k-baseline.json`：
...
见 `cangjie-hmos-doc-search-maintenance/records/ab-15k-after-rule-llm.json`：
...
- semantic capability gate：`../cangjie-hmos-doc-search-maintenance/scripts/run_semantic_capability_gate.py` 输出 `status: ok`。
- V3 regression gate：`../cangjie-hmos-doc-search-maintenance/scripts/run_v3_regression_gate.py --max-rows 500` 完成，`recall@5=0.4271`、`recall@10=0.6668`、`mrr=0.6245`；全量 `eval_queries_full.jsonl` 为 93486 条，直接全跑会耗时较长，脚本已新增 `--max-rows` 供门禁采样使用。
...
5. **门禁拆分**：`../cangjie-hmos-doc-search-maintenance/scripts/run_v3_regression_gate.py`、`../cangjie-hmos-doc-search-maintenance/scripts/run_semantic_capability_gate.py`；`run_release_eval.py` 文档指向二者。
...
- 用户态与维护态分层：`cangjie-hmos-doc-search/scripts/` 仅保留 README，原评测/AB/日志分析/种子同步/遗留 V1 等脚本已迁入 `cangjie-hmos-doc-search-maintenance/scripts/`。
- V1 `search.py` → `cangjie-hmos-doc-search-maintenance/scripts/_legacy_search_v1.py`。
