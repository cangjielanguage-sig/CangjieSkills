...
- 方式二：
    ...
    通过AVSession注册[HandleMediaKeyEvent](../../../cj-apis-multimedia_avsession/.overview.md)指令。该回调接口会直接转发媒体按键事件[KeyEvent](../../../cj-apis-multimodalInput-keyEvent/.overview.md)。应用需要自行识别按键事件的类型，并响应事件实现对应的功能。目前支持转发的按键事件类型如下：

  |按键类型([KeyCode](../../../cj-apis-multimodalInput-keyCode/.overview.md))| 功能说明|
  |------|-------------------------|
