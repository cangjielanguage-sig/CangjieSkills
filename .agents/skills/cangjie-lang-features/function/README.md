...
- 无参 Lambda：`{ => exprs }`
- 只有在分析 Lambda 前，期望函数类型已经由唯一调用目标或显式变量/参数类型确定时，参数类型才可省略；调用存在重载、重载选择依赖 Lambda 本身或上下文不能唯一确定函数类型时，必须为全部参数显式标注类型，并由对应 API 专题核对回调返回契约
- 返回类型**始终推断**，不可显式声明
...
- 回调是纯映射、谓词或比较器，且只读取参数或不可变值时，Lambda 通常最直接。
- Lambda 函数体允许包含多个表达式；是否改用普通函数或局部函数，应由复用、可读性、提前退出和状态边界决定，而不是把多语句本身视为非法。
- 需要累加、交换、多指针推进或跨步骤更新状态时，可选择显式循环或局部函数，使状态和控制流保持在调用点；不要为了使用高阶 API 而隐藏状态迁移。
- 选择 Lambda 后仍须检查第 5 节的捕获与逃逸规则；“语法可写”不代表该闭包可以作为值传递。
...
捕获 `var` 不等于 Lambda 一律不可用：立即调用仍是合法路径；但数组初始化器、map/filter/sort 回调等位置会把 Lambda 作为参数传递，因此不能传入捕获局部 `var` 的闭包。此时可绑定真正不可变的输入，或改用显式循环/局部函数完成状态处理。
