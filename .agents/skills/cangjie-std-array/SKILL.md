...
description: "仓颉标准库 Array 类型详细指南。当需要了解 Array 的构造、元素访问、切片、拼接、查找、排序、拷贝、反转、映射、扁平化等操作，或修复 HumanEval/算法题中的 Array.append/add、Array.sort、Repeat/repeat、length/size 等 Array 编译错误时，应使用此 Skill。"
---
...
> **HumanEval P0 自查**：`Array<T>` 没有 `.append()` / `.add()` / `.sort()` / `.length`，也没有 `.size()` 方法。动态收集元素用 `ArrayList<T>.add()`，返回 `Array<T>` 时最后 `.toArray()`；排序用 `import std.sort.*` 后全局 `sort(arr)`；重复初始化命名参数必须是小写 `repeat:`，不要写 `Repeat:`。
...
- 重复初始化必须写 `Array<T>(n, repeat: value)`，`repeat:` 小写；不要写 `Repeat:` / `item:`。
...
sort(arr, descending: true)      // 原地降序
```

> **注意**：`sort(arr)` 原地修改并返回 `Unit`；不要写 `let sorted = arr.sort()` 或 `let sorted = sort(arr)`。
