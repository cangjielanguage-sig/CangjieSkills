...
PYTHONDONTWRITEBYTECODE=1 python diff_doc_manifest.py \
...
PYTHONDONTWRITEBYTECODE=1 python generate_eval_candidates_from_doc_diff.py \
...
发布结论以 `run_release_eval.py` 输出为准。报告必须包含 `release-summary.json`、`release-report.md` 和各评测集的 `eval-health.json / summary.json / details.jsonl / failure-summary.json`。
...
- 发布前必须跑 `run_release_eval.py --index-dir /tmp/cangjie-index-llm`。
...
- 主评测集只接受审核后的 query。
- `generate_eval_candidates_from_doc_diff.py` 和 `analyze_search_logs.py` 产物只能进入候选池。
- `validate_eval_set.py` 出现 `missing_path` 时发布必须 blocked。
- 候选集失败不阻塞发布，但必须记录。
...
```bash
PYTHONDONTWRITEBYTECODE=1 python generate_appdev_eval_batch3_blind.py \
  --strict-blind-out eval_queries_user_appdev_blind_YYYYMMDD.jsonl \
  --strict-source strict-blind-YYYYMMDD
```

首跑后立即记录结果，作为严格 blind v1 基线。若后续根据该 blind 调整搜索规则或索引，它就只能作为回归集；下一次发布前需要生成新的时间戳 blind。
...
PYTHONDONTWRITEBYTECODE=1 python analyze_search_logs.py \
  --log /tmp/search-events.jsonl \
...
- 严格 blind：`eval_queries_user_appdev_blind_20260424.jsonl`，80 条，首跑 `success@1 = 0.875`、`success@5 = 1.0`
