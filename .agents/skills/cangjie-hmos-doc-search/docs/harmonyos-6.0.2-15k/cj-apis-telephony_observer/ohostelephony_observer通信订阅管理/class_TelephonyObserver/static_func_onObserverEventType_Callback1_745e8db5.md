...
public static func on(eventType: ObserverEventType,
    callback: Callback1Argument<DataConnectionStateInfo>, options!: ?ObserverOptions = None): Unit
```
...
|callback|[Callback1Argument](../../../cj-apis-base/.overview.md)\<[DataConnectionStateInfo](#class-dataconnectionstateinfo)>|是|-|回调函数。|
...
let callback4 = MyCallbackObj<DataConnectionStateInfo>(
    {
        val: DataConnectionStateInfo => AppLog.info(
            "[on cellularDataConnectionStateChange]: state = ${val.state.getValue()}, number = ${val.network.getValue()}"
        )
    })

TelephonyObserver.on(ObserverEventType.CellularDataConnectionStateChange, callback4)
```