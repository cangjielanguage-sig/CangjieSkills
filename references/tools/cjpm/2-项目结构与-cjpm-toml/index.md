...
| [2.2 基本 cjpm.toml（单模块）](2-2-基本-cjpm-toml-单模块.md) | 单模块 `cjpm.toml` 的 `[package]` 至少给出非空 `cjc-version = "1.1.3"`、`name`、`version` 和 `output-type`；可执行项目使用 `output-type = "executable"`。 |
