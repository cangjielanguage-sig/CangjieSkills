...
业务调用[preQuery](../cj-apis-asset_store/.overview.md)预查询后，没有通过[postQuery](../cj-apis-asset_store/.overview.md)释放资源。

**处理步骤**

业务调用[preQuery](../cj-apis-asset_store/.overview.md)预查询后，通过[postQuery](../cj-apis-asset_store/.overview.md)释放资源。
...
1. 先调用[remove](../cj-apis-asset_store/.overview.md)
2. 删除同别名的关键资产，再调用[add](../cj-apis-asset_store/.overview.md)重新写入。
3. 调用[add](../cj-apis-asset_store/.overview.md)时，指定参数AssetTag.CONFLICT_RESOLUTION的值为AssetConflictResolution.OVERWRITE
...
1. 用户尚未调用过[add](../cj-apis-asset_store/.overview.md)接口，进行asset数据库的创建，此时数据库还没创建出来，直接查询，会有该报错。
2. 数据库访问异常。

**处理步骤**

1. 用户先调用[add](../cj-apis-asset_store/.overview.md)接口，将asset数据库创建出来之后再进行查询。
2. 查看错误信息，排查数据库异常原因。
