...
1. 调用[createSymKeyGenerator](../cj-apis-crypto/.overview.md)，指定字符串参数'AES256'，创建密钥算法为AES、密钥长度为256位的对称密钥生成器（SymKeyGenerator）。

2. 调用[generateSymKey](../cj-apis-crypto/.overview.md)，随机生成对称密钥对象（SymKey）。

3. 调用[getEncoded](../cj-apis-crypto/.overview.md)，获取密钥对象的二进制数据。
...
1. 调用[createSymKeyGenerator](../cj-apis-crypto/.overview.md)，指定字符串参数'SM4_128'，创建密钥算法为SM4、密钥长度为128位的对称密钥生成器（SymKeyGenerator）。
