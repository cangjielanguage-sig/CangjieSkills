...
- 在预览时，图像旋转角度与屏幕显示旋转角度（[Display.rotation](../../cj-apis-display/.overview.md)）相关。具体开发指导：[创建会话](#创建会话) > [预览](#预览)
- 在拍照、录像时，图像旋转角度与设备重力方向（即[设备旋转角度](#计算设备旋转角度)）相关。

    录像开发指导：[创建会话](#创建会话) > [计算设备旋转角度](#计算设备旋转角度) > [录像](#录像)

详细的API参考说明，请参见[Camera API文档](../../cj-apis-multimedia-camera/.overview.md)。
...
2. 创建Session会话。
    ...
    相机使用预览等功能前，均需创建相机会话，调用[CameraManager](../../cj-apis-multimedia-camera/.overview.md)类中的[createSession](../../cj-apis-multimedia-camera/.overview.md)方法创建一个会话，创建会话时需指定创建[SceneMode](../../cj-apis-multimedia-camera/.overview.md)为NORMAL_PHOTO或NORMAL_VIDEO，创建的session处于拍照或者录像模式。
