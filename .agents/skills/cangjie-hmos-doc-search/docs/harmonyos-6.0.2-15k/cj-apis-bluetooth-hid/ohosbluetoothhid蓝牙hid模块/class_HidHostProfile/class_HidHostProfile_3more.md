...
**功能：** 使用HidHostProfile方法之前需要创建该类的实例进行操作，通过createHidHostProfile()方法构造此实例。
...
try {
    ...
    let retArray = hidProfile.getConnectedDevices()
...
try {
    let hidProfile = createHidHostProfile()
    let ret = hidProfile.getConnectionState("XX:XX:XX:XX:XX:XX")
} catch (e: BusinessException) {
    Hilog.info(0, "Bluetooth", "errCode: ${e.code}, errMessage: ${e.message}")
}
```