...
系统预设的默认音频焦点策略，主要依据音频流类型（即播放流的[StreamUsage](../../../cj-apis-multimedia-audio/.overview.md)和录制流的[SourceType](../../../cj-apis-multimedia-audio/.overview.md)）及音频流启动的顺序进行决策。
...
针对同一应用创建的多个音频流，应用可通过设置[焦点模式（InterruptMode）](../../../cj-apis-multimedia-audio/.overview.md)，选择由应用自主管控，或由系统统一管理。
...
- 若[使用AVPlayer开发音频播放功能](../../../cj-media-kit-using-avplayer-for-playback/.overview.md)，则可以通过修改AVPlayer的[audioInterruptMode](../../../cj-apis-multimedia_media/.overview.md)属性进行设置。

- 若[使用AudioRenderer开发音频播放功能](./cj-using-audiorenderer-for-playback.md)，则可以调用[setInterruptMode](../../../cj-apis-multimedia-audio/.overview.md)函数进行设置。