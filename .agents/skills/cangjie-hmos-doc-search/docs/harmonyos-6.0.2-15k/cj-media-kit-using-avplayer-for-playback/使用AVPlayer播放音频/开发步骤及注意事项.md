...
详细的API说明请参见[AVPlayer API参考](../../cj-apis-multimedia_media/.overview.md)。
...
3. 设置资源：设置属性url，AVPlayer进入initialized状态。
   ...
   >
   > - 如果使用本地资源播放，必须确认资源文件可用，并使用应用沙箱路径访问对应资源，请参见[获取应用文件路径](../../cj-application-context-stage/.overview.md)。应用沙箱的介绍及如何向应用沙箱推送文件，请参见[文件管理](../../cj-app-sandbox-directory/.overview.md)。
   > - 如果使用网络播放路径，需[声明权限](../../cj-declare-permissions/.overview.md)：ohos.permission.INTERNET。
   > - 如果使用ResourceManager.getRawFd打开HAP资源文件描述符，使用方法请参见[ResourceManager API参考](../../cj-apis-resource_manager/.overview.md)。
   > - 需要使用[支持的播放格式与协议](../../cj-media-kit-intro/.overview.md)。
   > 此外，如果需要设置音频渲染信息，则只允许在initialized状态下，第一次调用prepare()之前设置，以便音频渲染器信息在之后生效。若媒体源包含视频，则usage默认值为STREAM_USAGE_MOVIE，否则usage默认值为STREAM_USAGE_MUSIC。rendererFlags默认值为0。若默认usage不满足需求，则须主动配置[AudioRendererInfo](../../cj-apis-multimedia-audio/.overview.md)。
