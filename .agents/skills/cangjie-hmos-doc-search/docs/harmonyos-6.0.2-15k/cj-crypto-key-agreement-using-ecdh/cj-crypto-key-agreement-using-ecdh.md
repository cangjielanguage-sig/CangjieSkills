...
func ecdh() {
    ...
    let priKeyArray: Array<UInt8> = [48, 49, 2, 1, 1, 4, 32, 115, 56, 137, 35, 207, 0, 60, 191, 90, 61, 136, 105, 210,
        ...
        206, 61, 3, 1, 7]
    let eccGen = createAsyKeyGenerator('ECC256')
    // 外部传入的公私钥对A。
    let keyPairA = eccGen.convertKey(DataBlob(pubKeyArray), DataBlob(priKeyArray))
    // 内部生成的公私钥对B。
    let keyPairB = eccGen.generateKeyPair()
    let eccKeyAgreement = createKeyAgreement('ECC256')
    // 使用A的公钥和B的私钥进行密钥协商。
    let secret1 = eccKeyAgreement.generateSecret(keyPairB.priKey, keyPairA.pubKey)
    // 使用A的私钥和B的公钥进行密钥协商。
    let secret2 = eccKeyAgreement.generateSecret(keyPairA.priKey, keyPairB.pubKey)
    // 两种协商的结果应当一致。
    if (secret1.data.toString() == secret2.data.toString()) {
        AppLog.info('ecdh success')
        AppLog.info('ecdh output is ${secret1.data}')
    } else {
        AppLog.error('ecdh result is not equal')
    }
}
```
