...
1. 调用[createAsyKeyGenerator](../cj-apis-crypto/.overview.md)生成密钥算法为DH、采用知名安全素数群modp1536的非对称密钥（KeyPair）。
...
2. 调用[createKeyAgreement](../cj-apis-crypto/.overview.md)，指定字符串参数'DH_modp1536'，创建密钥算法为DH、采用知名安全素数群modp1536的密钥协议生成器（KeyAgreement）。

3. 调用[generateSecret](../cj-apis-crypto/.overview.md)，基于传入的私钥（KeyPair.priKey）与公钥（KeyPair.pubKey）进行密钥协商，返回共享秘密。
