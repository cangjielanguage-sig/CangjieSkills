...
先把 `seed/` 复制到隔离工作目录。它是一个已有的仓颉 1.0.5 多模块源码审计器，包含编译期宏、`std.ast` 源码分析、反射式规则注册、C FFI、确定性报告和命令行应用。项目目前可以编译，但一次错误合并引入了多处互不相关的逻辑回归。不得修改任务目录中的 `seed/`。
...
- 工具链：仓颉 `1.0.5`、cjnative、Windows x64；仅使用 `std.*`，不联网。
