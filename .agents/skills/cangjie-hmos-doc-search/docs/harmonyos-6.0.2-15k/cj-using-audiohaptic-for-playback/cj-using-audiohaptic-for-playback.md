...
使用AudioHaptic播放音频并同步开启振动，涉及到音频及振动资源的管理、音频时延模式及音频流使用类型的配置、音振播放器的创建及管理等。本开发指导将以一次音振协同播放的过程为例，向开发者讲解如何使用AudioHaptic进行音振协同播放，建议配合[AudioHaptic的API说明](../cj-apis-multimedia-audio_haptic/.overview.md)阅读。

### 权限申请

如果应用创建的AudioHapticPlayer需要触发振动，必须校验应用是否拥有 `ohos.permission.VIBRATE` 权限。

1. [声明权限](../cj-declare-permissions/.overview.md)。
2. [向用户申请授权](../cj-request-user-authorization/.overview.md)。

### 开发步骤及注意事项

1. 获取音振管理器实例，并注册音频及振动资源，资源支持情况请参见[AudioHapticManager](../cj-apis-multimedia-audio_haptic/.overview.md)。
...
2. 设置音振播放器参数，各参数作用请参见[AudioHapticManager](../cj-apis-multimedia-audio_haptic/.overview.md)。
