...
[音频流](./cj-audio-kit-intro.md#音频流介绍)类型是定义音频数据播放和录制方式的关键属性。对于播放流，其类型由[StreamUsage](../../cj-apis-multimedia-audio/.overview.md)确定；对于录制流，则由[SourceType](../../cj-apis-multimedia-audio/.overview.md)决定。音频流类型对音量控制、音频焦点管理以及输入/输出设备的选择具有决定性影响。
...
下表中列举常用的播放音频流类型，由[StreamUsage](../../cj-apis-multimedia-audio/.overview.md)定义。

| 音频流使用类型（StreamUsage） | 适用场景 |
| ---------- | ---------- |
| STREAM_USAGE_MUSIC | 适用于播放音乐，同样适用于其他媒体场景，如[使用SoundPool](../../cj-media-kit-using-soundpool-for-playback/.overview.md)播放简短音效等。 |
...
下表中列举常用的录制音频流类型，由[SourceType](../../cj-apis-multimedia-audio/.overview.md)定义。
