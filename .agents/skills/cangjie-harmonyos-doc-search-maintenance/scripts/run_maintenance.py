...
WORKFLOW_VERSION = "v3-stage4-15k"
GRAPHIFY_DATA_DIR = MAINTENANCE_DIR.parent / "knowledge-graph-template" / "data"
...
def run_fusion_ab_gate(index_dir: Path, run_dir: Path) -> list[str]:
    """对候选索引跑 V3/graphify/fusion 独立 AB；要求 fusion recall 不低于单引擎。"""
...
def main() -> None:
    ...
    parser.add_argument("--eval-set", default=str(DOC_CARD_DIR / "evals" / "search" / "eval_queries.jsonl"))
    ...
    if args.fusion_ab_gate:
        if not GRAPHIFY_DATA_DIR.is_dir():
            report = {
                "timestamp": timestamp,
                ...
                "api_audit": api_audit_summary,
                "benchmarks": benchmarks,
                "gate_failure_reasons": [f"knowledge-graph data 目录不存在: {GRAPHIFY_DATA_DIR}"],
            }
            write_text(run_dir / "report.json", json.dumps(report, ensure_ascii=False, indent=2))
            write_text(run_dir / "report.md", markdown_summary(report))
            append_changelog(timestamp, report)
