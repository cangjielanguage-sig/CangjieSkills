...
5. 执行任务要求的 build/test/run 与专用工具门禁。只格式化本次修改的生产 `.cj` 文件；没有项目命令时逐文件运行 `cjfmt -f <file>`，不要格式化不可修改测试或无关文件。最终重新执行完整验收。
...
```text
python -B scripts/search_docs.py --query "ArrayList reverse" --query "Int64 writeBigEndian" --query "cjpm run arguments" --max-results 2
```
...
```text
python -B scripts/search_docs.py --node language.collections --view indexes
python -B scripts/search_docs.py "集合类型" --view indexes
```
...
```text
python -B scripts/search_docs.py --query "ArrayList reverse" --query "HashMap tuple iteration" --view leaves
```
...
- 当前 Skills 知识库适配仓颉 1.1.x 版本，如果环境中工具链是其他版本，出现差异时请以工具链反馈为准。
