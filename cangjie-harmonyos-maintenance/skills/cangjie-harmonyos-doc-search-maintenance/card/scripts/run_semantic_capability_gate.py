...
本脚本独立于 run_v3_regression_gate（V3 自举门禁），专门验证 fusion 引擎
的语义组合能力是否优于单引擎。门禁逻辑：任何 split 中 fusion recall
低于 V3 或 graphify 即判失败。

与 run_v3_regression_gate.py 拆分的原因：结构化召回（V3）和语义融合能力
属于不同维度，不应绑在同一阈值上。
"""
...
CARD_SCRIPTS_DIR = CARD_DIR / "scripts"
CARD_EVALS_DIR = CARD_DIR / "evals"
GRAPH_DATA = DOC_GRAPH_DIR / "data"
...
def main() -> None:
    ...
    parser.add_argument("--eval-dir", default=str(CARD_EVALS_DIR))
    ...
    ab_script = CARD_SCRIPTS_DIR / "run_ab_eval.py"
    ...
    reasons: list[str] = []
    # 1e-9 浮点容差：避免因为浮点精度导致 fusion==v3 误判为低于
