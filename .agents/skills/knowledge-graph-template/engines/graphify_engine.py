...
QUERY_EXPANSIONS = {
    "下拉刷新": ["refresh", "cj-scroll-swipe-refresh", "Refresh"],
    "刷新": ["refresh", "cj-scroll-swipe-refresh", "Refresh"],
    "列表": ["list", "cj-scroll-swipe-list", "List"],
    "长列表": ["list", "cj-scroll-swipe-list", "lazy", "performance"],
    "网络列表": ["list", "http", "httprequest", "cj-apis-net-http", "cj-http-request"],
    "网络请求": ["http", "httprequest", "cj-apis-net-http", "cj-http-request"],
    "网络": ["http", "httprequest", "cj-apis-net-http", "cj-http-request"],
    "卡顿": ["performance", "lazy", "onReachEnd", "cachedCount", "List"],
    "滑动卡顿": ["performance", "lazy", "onReachEnd", "cachedCount", "List"],
    "优化": ["performance", "lazy"],
    "原生双向传数据": ["WebView", "WebviewController", "registerJavaScriptProxy", "javaScriptProxy"],
    "双向传数据": ["WebView", "WebviewController", "registerJavaScriptProxy", "javaScriptProxy"],
    "原生": ["native", "registerJavaScriptProxy"],
    "webview": ["WebviewController", "registerJavaScriptProxy", "ArkWeb"],
    "web view": ["WebviewController", "registerJavaScriptProxy", "ArkWeb"],
    # 跨生态类比（15k 目录扁平化，用路径片段拉召回）
    "recyclerview": [
        "lazyforeach",
        "list",
    ...
    "swiperefreshlayout": ["refresh", "list", "cj-scroll-swipe-refresh", "cj-scroll-swipe-list"],
}
...
def tokenize_query(query: str) -> list[str]:
    ...
    for trigger, expansions in QUERY_EXPANSIONS.items():
        if trigger.lower() in lowered:
            tokens.extend(e.lower() for e in expansions)
...
def expanded_query(query: str) -> str:
    """把中文自然语言查询补成图谱里更常见的 API/路径关键词。"""
    tokens = tokenize_query(query)
    extras = [t for t in tokens if t not in query.lower() and len(t) >= 3]
    if not extras:
        return query
...
def normalize_path(path: str) -> str:
    """标准化文件路径。"""
    path = path.replace("\\", "/")
    for prefix in ["harmonyos-6.0.2-15k", "lang-features", "std", "stdx", "tools"]:
