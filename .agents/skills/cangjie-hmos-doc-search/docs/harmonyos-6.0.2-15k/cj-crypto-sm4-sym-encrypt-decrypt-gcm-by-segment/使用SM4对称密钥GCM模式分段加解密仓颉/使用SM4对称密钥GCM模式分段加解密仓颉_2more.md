...
2. 调用[createCipher](../../cj-apis-crypto/.overview.md)，指定字符串参数'SM4_128|GCM|PKCS7'，创建对称密钥类型为SM4_128、分组模式为GCM、填充模式为PKCS7的Cipher实例，用于完成加密操作。

3. 调用[init](../../cj-apis-crypto/.overview.md)，设置模式为加密（CryptoMode.ENCRYPT_MODE），指定加密密钥（SymKey）和GCM模式对应的加密参数（GcmParamsSpec），初始化加密Cipher实例。

4. 将一次传入数据量设置为20字节，多次调用[update](../../cj-apis-crypto/.overview.md)，更新数据（明文）。
...
6. 读取[GcmParamsSpec](../../cj-apis-crypto/.overview.md).authTag作为解密的认证信息。
