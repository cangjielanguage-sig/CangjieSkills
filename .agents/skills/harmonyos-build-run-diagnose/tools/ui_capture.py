...
用法:
    # 基础采集（截图 + 控件树）
    python -B ui_capture.py [--bundle BUNDLE] [--ability ABILITY] [--out DIR] [--no-launch]
    python -B ui_capture.py --emulator 5555

    # 交互验证（执行场景 → 二次采集 → 差异报告）
    python -B ui_capture.py --scenario scenario.json --out ./ui_capture_output
    python -B ui_capture.py --scenario scenario.json --emulator 5555
...
#     {"action": "snapshot", "label": "中间状态", "assertions": [
#       {"type": "text_equals", "target": {"key": "counter"}, "expected": "1"}
#     ]}
#   ],
#   "assertions": [
...
def resolve_hdc(explicit: Optional[str] = None) -> str:
    ...
    raise SystemExit(
        "hdc not found. Pass --hdc or install DevEco Studio toolchains. "
...
def _prepare_cmd(cmd: str) -> str:
    if cmd == "hdc" or cmd.startswith("hdc "):
        return _quote_cmd(HDC_CMD) + cmd[3:]
    return cmd
...
def command_ok(code: int, out: str) -> bool:
    """Backward-compatible wrapper around shared hdc result parsing."""
    return hdc_command_ok(code, out)
...
def connect_emulator(addr: str) -> str:
    ...
    print(f"  {out}")
    if command_ok(code, out) and ("Connect OK" in out or "already connected" in out.lower()):
        print(f"[OK] 模拟器已连接: {addr}")
        return addr
    # 即使输出不含预期关键字，也检查 list targets 确认
    target_code, targets_out = run_result("hdc list targets")
    if command_ok(target_code, targets_out) and addr in targets_out:
        print(f"[OK] 模拟器已连接: {addr}")
        return addr
...
def check_device(emulator_addr: Optional[str] = None) -> str:
    ...
    if not command_ok(code, out):
        print("[ERROR] hdc list targets failed.")
        print(f"  hdc: {HDC_CMD}")
        print(f"  reason: {hdc_failure_reason(code, out)}")
        print(f"  output: {out}")
        sys.exit(1)
    targets = [
        t.strip() for t in out.splitlines()
        if t.strip() and "Empty" not in t and "not recognized" not in t
    ]
    if not targets:
...
def install_hap(hap_path: str) -> bool:
    ...
    if hap_path and os.path.isfile(hap_path):
        ...
        print(f"  {out}")
        if not command_ok(code, out):
            print(f"[ERROR] HAP install failed: {hdc_failure_reason(code, out)}")
            return False
        return True
    if hap_path:
...
def launch_app(bundle: str, ability: str, module: Optional[str] = None) -> bool:
    ...
    print(f"[INFO] 启动 {bundle}/{ability}{(' module=' + module) if module else ''} ...")
    code, out = run_result(f"hdc shell aa start -a {ability} -b {bundle}{module_arg}")
    if out:
        print(f"  {out}")
    time.sleep(3)
    if not command_ok(code, out):
        print(f"[ERROR] App launch command failed: {hdc_failure_reason(code, out)}")
        return False
    return True
...
def capture_screenshot(out_dir: str) -> str:
    ...
    for name, cmd, skip_on_error in methods:
        ...
        if skip_on_error and "error" in out.lower():
            continue
        recv_code, recv_out = run_result(f'hdc file recv {device_path} "{local_path}"')
        run(f"hdc shell rm -f {device_path}")
        if not command_ok(recv_code, recv_out):
            continue
...
def dump_layout(out_dir: str) -> str:
    ...
    if not command_ok(code, out):
        print(f"[WARN] dumpLayout 执行失败: {hdc_failure_reason(code, out)}; {out}")
        return local_path
    recv_code, recv_out = run_result(f'hdc file recv {device_json} "{local_path}"')
    run(f"hdc shell rm {device_json}")
    if not command_ok(recv_code, recv_out):
        print(f"[WARN] 控件树拉取失败: {hdc_failure_reason(recv_code, recv_out)}")
        return local_path
...
def validate_scenario(scenario: dict) -> None:
    """Validate the dependency-free subset needed for actionable error messages."""
    ...
    for index, step in enumerate(steps, start=1):
        if not isinstance(step, dict):
            raise ValueError(f"scenario.steps[{index}] must be an object")
        if not isinstance(step.get("action"), str) or not step.get("action"):
            raise ValueError(f"scenario.steps[{index}].action must be a non-empty string")
        if step.get("action") == "snapshot" and not isinstance(step.get("assertions", []), list):
            raise ValueError(f"snapshot step {index}.assertions must be an array")
    for scope, items in [("scenario.assertions", assertions)] + [
        (f"snapshot step {index}.assertions", step.get("assertions", []))
        for index, step in enumerate(steps, start=1)
        if step.get("action") == "snapshot"
    ]:
        for assertion_index, assertion in enumerate(items, start=1):
            if not isinstance(assertion, dict):
                raise ValueError(f"{scope}[{assertion_index}] must be an object")
            if not isinstance(assertion.get("type"), str) or not assertion.get("type"):
                raise ValueError(f"{scope}[{assertion_index}].type must be a non-empty string")


def _snapshot_label(value: object, step_idx: int) -> str:
    """Return a readable label that cannot escape the output directory."""
    label = str(value or f"step_{step_idx}").strip()
    label = re.sub(r'[<>:"/\\|?*\x00-\x1f]', "_", label)
    return label or f"step_{step_idx}"
...
def execute_step(step: dict, flat_nodes: List[dict], screen_w: int, screen_h: int,
    ...
    if action == "snapshot":
        label = _snapshot_label(step.get("label"), step_idx)
        snap_dir = os.path.join(out_dir, f"snapshot_{label}")
        os.makedirs(snap_dir, exist_ok=True)
        (_, layout_path, _, screenshot_ok, layout_ok, bundle_ok) = capture_once(snap_dir, bundle)
        print(f"  [STEP {step_idx}] 中间快照: {snap_dir}")
        result["snapshot"] = snap_dir
        local_assertions = step.get("assertions", [])

        if not screenshot_ok and not layout_ok:
            result["detail"] = "中间快照失败：截图和控件树均未采集到"
            return result
        if layout_ok and bundle and not bundle_ok:
            result["detail"] = f"中间快照不是目标应用：控件树未包含 bundle {bundle}"
            return result
        if local_assertions and not layout_ok:
            result["detail"] = "中间快照局部断言无法执行：控件树未采集到"
            return result

        if local_assertions:
            baseline_path = os.path.join(out_dir, "layout.json")
            reference_path = baseline_path if os.path.isfile(baseline_path) else layout_path
            local_diff = diff_layouts(reference_path, layout_path)
            result["assertions"] = evaluate_assertions(
                local_assertions, reference_path, layout_path, local_diff)
            passed = sum(1 for item in result["assertions"] if item["passed"])
            total = len(result["assertions"])
            failures = [item for item in result["assertions"] if not item["passed"]]
            if failures:
                failure_text = "; ".join(
                    f"{item['assertion'].get('message') or item['assertion'].get('type')}: {item['detail']}"
                    for item in failures
                )
                result["detail"] = f"快照保存至 {snap_dir}；局部断言 {passed}/{total} 通过；{failure_text}"
                return result
            result["detail"] = f"快照保存至 {snap_dir}；局部断言 {passed}/{total} 通过"
        else:
...
def _refresh_step_layout(out_dir: str) -> tuple:
    ...
    dump_code, dump_out = run_result(f"hdc shell uitest dumpLayout -p {device_json}")
    if not command_ok(dump_code, dump_out):
        print(f"  [WARN] 控件树刷新失败: {hdc_failure_reason(dump_code, dump_out)}")
        return [], 1080, 2340
    recv_code, recv_out = run_result(f'hdc file recv {device_json} "{local_tmp}"')
    run(f"hdc shell rm -f {device_json}")
    if not command_ok(recv_code, recv_out):
        print(f"  [WARN] 控件树刷新拉取失败: {hdc_failure_reason(recv_code, recv_out)}")
        return [], 1080, 2340
...
def generate_interaction_report(scenario: dict, step_results: List[dict],
    ...
    snapshot_results = [r for r in step_results if r.get("action") == "snapshot" and r.get("assertions")]
    if snapshot_results:
        lines.append("### 中间快照局部断言")
        for result in snapshot_results:
            lines.append(f"- `{result.get('snapshot', '')}`")
            for item in result["assertions"]:
                status = "PASS" if item["passed"] else "FAIL"
                assertion = item["assertion"]
                label = assertion.get("message") or assertion.get("type", "")
                lines.append(f"  - {status} {label}: {item['detail']}")
        lines.append("")
...
def main():
    ...
    if not args.emulator:
        args.emulator = getattr(device_cfg, "target", None) if device_cfg else None
    if not args.bundle:
    ...
    while layout_ok and args.bundle and not bundle_ok and not args.no_launch and retry_count < args.foreground_retries:
        ...
        screenshot_path, layout_path, summary_path, screenshot_ok, layout_ok, bundle_ok = capture_once(args.out, args.bundle)
    if not screenshot_ok and not layout_ok:
        print("[ERROR] UI capture failed: neither screenshot nor layout was collected.")
        sys.exit(3)
    if layout_ok and args.bundle and not bundle_ok:
        print(f"[ERROR] UI capture did not reach target bundle: {args.bundle}")
        print("        The captured layout appears to be launcher/system UI. Retry with --module, --no-launch after manual aa start, or inspect hilog.")
        sys.exit(4)
    ...
    if args.scenario:
        ...
        if not after_screenshot_ok and not after_layout_ok:
            print("[ERROR] UI capture failed after interaction: neither screenshot nor layout was collected.")
            sys.exit(3)
        if after_layout_ok and args.bundle and not after_bundle_ok:
            print(f"[ERROR] Post-interaction capture did not contain target bundle: {args.bundle}")
            sys.exit(4)
        ...
        if step_failed or assertion_failed:
            print("[ERROR] Interaction validation failed.")
            sys.exit(5)
    else:
        print(f"\n{'='*50}")
