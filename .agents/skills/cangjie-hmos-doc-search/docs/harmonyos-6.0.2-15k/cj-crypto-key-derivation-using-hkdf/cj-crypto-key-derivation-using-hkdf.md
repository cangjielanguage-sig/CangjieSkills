...
1. 构造[HKDFSpec](../cj-apis-crypto/.overview.md)对象，作为密钥派生参数进行密钥派生。

    HKDFSpec是[KdfSpec](../cj-apis-crypto/.overview.md)的子类，需要指定：
...
2. 调用[createKdf](../cj-apis-crypto/.overview.md)，指定字符串参数'HKDF|SHA256|EXTRACT_AND_EXPAND'，创建密钥派生算法为HKDF、HMAC函数摘要算法为SHA256、模式为提取和拓展的密钥派生函数对象（Kdf）。

3. 输入HKDFSpec对象，调用[generateSecret](../cj-apis-crypto/.overview.md)进行密钥派生。
