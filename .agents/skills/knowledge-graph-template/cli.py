...
def cmd_build_subgraph(args):
    ...
    if seeds_path:
        try:
            ...
            print(f"  载入 V3 种子: {seeds.get('node_count', 0)} 节点 / {seeds.get('edge_count', 0)} 边 (from {seeds_path})")
        except Exception as e:
            print(f"  载入种子失败 ({e})，按无种子模式继续")
            seeds = None
    ...
    if doc_files:
        ...
        api_base = getattr(args, 'api_base', None) or os.environ.get("OPENAI_BASE_URL")
        ...
        if llm_provider != "skip" and not api_key:
            raise SystemExit("LLM provider 已启用，但缺少 API key；请设置 OPENAI_API_KEY 或传入 --api-key")
        if llm_provider == "openai" and (not api_base or not model):
            raise SystemExit("openai provider 需要 OPENAI_BASE_URL/OPENAI_MODEL 或 --api-base/--model")
        ...
        if llm_provider != "skip" and api_key:
            ...
            try:
                ...
                sem_result = extract_docs_with_llm_sync(
                    ...
                    config=config,
                    deep_mode=deep_mode,
                    chunk_size=getattr(args, "chunk_size", 22) or 22,
                    max_concurrent=getattr(args, "max_concurrent", 3) or 3,
                    use_cache=True,
                    seeds=seeds,
                )
...
def main():
    ...
    p_build_subgraph.add_argument("--seeds", help="V3 种子 JSON 路径（sync_v3_to_graph.py 产物），让 LLM 复用 V3 节点 id")
    p_build_subgraph.add_argument("--chunk-size", type=int, default=22, help="每 chunk 文件数（默认 22，deepseek 建议 6-10）")
    p_build_subgraph.add_argument("--max-concurrent", type=int, default=3, help="最大并发 chunk 数")
    p_build_subgraph.add_argument("--timeout", type=int, default=120, help="单次 LLM 请求超时秒数")
