...
public static func on(eventType: ObserverEventType,
    callback: Callback1Argument<NetworkState>, options!: ?ObserverOptions = None): Unit
```
...
|callback|[Callback1Argument](../../../cj-apis-base/.overview.md)\<[NetworkState](cj-apis-telephony_radio.md#class-networkstate)>|是|-|回调函数。|
...
let callback1 = MyCallbackObj<NetworkState>(
    {
        val: NetworkState => AppLog.info(
            "[on networkStateChange] data.longOperatorName: ${val.longOperatorName}, data.shortOperatorName: ${val.shortOperatorName}, data.isRoaming: ${val.isRoaming}"
        )
    })

TelephonyObserver.on(ObserverEventType.NetworkStateChange, callback1)
```