...
1. 构造[PBKDF2Spec](../cj-apis-crypto/.overview.md)对象，作为密钥派生参数进行密钥派生。

    PBKDF2Spec是[KdfSpec](../cj-apis-crypto/.overview.md)的子类，需要指定：
...
2. 调用[createKdf](../cj-apis-crypto/.overview.md)，指定字符串参数'PBKDF2|SHA256'，创建密钥派生算法为PBKDF2、HMAC函数摘要算法为SHA256的密钥派生函数对象（Kdf）。

3. 输入PBKDF2Spec对象，调用[generateSecret](../cj-apis-crypto/.overview.md)进行密钥派生。
