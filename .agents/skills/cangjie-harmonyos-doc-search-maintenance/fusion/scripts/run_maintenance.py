#!/usr/bin/env python3
"""运行 fusion 分区的重建、评测与记录沉淀流程。

这是 fusion 分区维护的主入口脚本，流水线步骤与 card 分区基本一致：
1. 构建 rule 和 rule+llm 两份 V3 卡片索引（调用 card/builder 模块）
2. 运行 LLM 增强完整度门禁 + API 覆盖率审计
3. 运行 V3 自举评测 + 可选 OpenViking 远端基线
4. LLM 回归门禁
5. 可选 fusion AB 门禁
6. 选择最优发布模式并同步索引到 doc-card/index（索引仍发布到 doc-card）

fusion 分区与 card 分区的区别：评测集来源、记录存放路径、工作流版本号。
两者共用 card/builder 和 card/scripts 的构建逻辑，通过跨分区 sys.path 引入。
"""
...
CARD_BUILDER_DIR = MAINTENANCE_DIR / "card" / "builder"
CARD_SCRIPTS_DIR = MAINTENANCE_DIR / "card" / "scripts"
FUSION_RECORDS_DIR = FUSION_DIR / "records"
FUSION_EVALS_DIR = FUSION_DIR / "evals"
FUSION_RUN_HISTORY_DIR = FUSION_RECORDS_DIR / "run-history"
FUSION_BASELINES_DIR = FUSION_RECORDS_DIR / "baselines"
FUSION_CHANGELOG_PATH = FUSION_RECORDS_DIR / "changelog.md"
WORKFLOW_VERSION = "v3-stage4-15k-fusion"
API_GATE_THRESHOLD = 1.0  # API 覆盖率门禁阈值：coverage_ratio 必须 >= 1.0
DEFAULT_LLM_CARD_TYPES = "task,api,example,doc"

# 跨分区依赖：构建逻辑来自 card 分区，评测逻辑来自本分区
sys.path.insert(0, str(CARD_BUILDER_DIR))
sys.path.insert(0, str(CARD_SCRIPTS_DIR))
...
def llm_full_enrichment_gate(manifest: dict, llm_card_types: tuple[str, ...]) -> list[str]:
    """LLM 增强完整度门禁：检查每种卡片类型的 enriched/succeeded 数量与总数是否一致。"""
...
def append_changelog(timestamp: str, report: dict) -> None:
    ...
    existing = FUSION_CHANGELOG_PATH.read_text(encoding="utf-8") if FUSION_CHANGELOG_PATH.exists() else "# doc-search Maintenance Changelog\n\n"
    if not existing.endswith("\n"):
        existing += "\n"
    existing += line + "\n"
    FUSION_CHANGELOG_PATH.write_text(existing, encoding="utf-8")


def run_fusion_ab_gate(index_dir: Path, run_dir: Path) -> list[str]:
    """对候选索引跑 V3/doc-graph/fusion 独立 AB；要求 fusion recall 不低于单引擎。

    通过子进程调用本分区的 run_ab_eval.py，使用 1e-9 浮点容差避免精度问题。
    评测集来源为 fusion/evals 目录。"""
    out_json = run_dir / "fusion-ab-report.json"
    cmd = [
        sys.executable,
        str(SCRIPT_DIR / "run_ab_eval.py"),
        "--eval-dir",
        str(FUSION_EVALS_DIR),
        "--index-dir",
        str(index_dir),
        "--graph-dir",
        str(DOC_GRAPH_DIR / "data"),
        "--splits",
...
def main() -> None:
    ...
    parser.add_argument("--eval-set", default=str(FUSION_EVALS_DIR / "eval_queries.jsonl"))
    ...
    parser.add_argument("--llm-cache-dir", default=str(FUSION_RECORDS_DIR / "llm-cache"))
    ...
    run_dir = FUSION_RUN_HISTORY_DIR / timestamp
    ...
    if args.fusion_ab_gate:
        graph_data_dir = DOC_GRAPH_DIR / "data"
        if not graph_data_dir.is_dir():
            report = {
                ...
                "api_audit": api_audit_summary,
                "benchmarks": benchmarks,
                "gate_failure_reasons": [f"doc-graph data 目录不存在: {graph_data_dir}"],
            }
            write_text(run_dir / "report.json", json.dumps(report, ensure_ascii=False, indent=2))
            write_text(run_dir / "report.md", markdown_summary(report))
            append_changelog(timestamp, report)
        ...
        subprocess.run(
            [
                sys.executable,
                str(CARD_SCRIPTS_DIR / "sync_v3_to_graph.py"),
                "--index-dir",
    ...
    write_api_audit_report(
        selected_api_audit,
        FUSION_RECORDS_DIR / "api-coverage" / f"{timestamp}.json",
        FUSION_RECORDS_DIR / "api-coverage" / f"{timestamp}.md",
        latest_json=FUSION_RECORDS_DIR / "api-coverage" / "latest.json",
        latest_md=FUSION_RECORDS_DIR / "api-coverage" / "latest.md",
    )
    ...
    write_text(run_dir / "report.json", json.dumps(report, ensure_ascii=False, indent=2))
    write_text(run_dir / "report.md", markdown_summary(report))
    write_text(FUSION_BASELINES_DIR / f"{timestamp}.json", json.dumps(report, ensure_ascii=False, indent=2))
    write_text(FUSION_BASELINES_DIR / "latest.json", json.dumps(report, ensure_ascii=False, indent=2))
    append_changelog(timestamp, report)

    print(json.dumps({"run_dir": str(run_dir), "baseline": str(FUSION_BASELINES_DIR / "latest.json")}, ensure_ascii=False, indent=2))
