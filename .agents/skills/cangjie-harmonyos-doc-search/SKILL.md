...
description: "鸿蒙应用开发文档检索工具，遇到不熟悉的UI组件/系统能力API/框架机制/状态管理时或者基于构建报错日志查找所需知识时使用"
---
...
遇到不熟悉的鸿蒙 UI 组件、系统能力 API、状态管理或框架机制时，执行 `search.py` 检索相关文档；或者在构建报错日志中遇到不明问题时，基于日志分析结果使用 `search.py` 查找所需知识。
