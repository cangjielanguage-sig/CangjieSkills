...
2. 指定用户认证相关参数[AuthParam](../cj-apis-user_auth/.overview.md)（包括挑战值、认证类型[UserAuthType](../cj-apis-user_auth/.overview.md)列表和认证等级[AuthTrustLevel](../cj-apis-user_auth/.overview.md)），获取认证对象[UserAuthInstance](../cj-apis-user_auth/.overview.md)，并调用[UserAuthInstance.start](../cj-apis-user_auth/.overview.md)发起认证。此步骤详细说明请参见[发起认证](./cj-start-authentication.md)。
3. 通过使用已经成功发起认证的UserAuthInstance对象调用[UserAuthInstance.cancel](../cj-apis-user_auth/.overview.md)接口取消本次认证。
