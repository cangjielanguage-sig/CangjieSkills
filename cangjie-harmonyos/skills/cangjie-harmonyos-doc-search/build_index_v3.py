...
def enrich_cards_with_llm(
    ...
    def process_rows(card_type: str, rows: list[dict], allowed_keys: list[str]) -> list[dict]:
        ...
        target_rows = rows
        print(f"[rule+llm] start {card_type}: total={len(target_rows)}", file=sys.stderr)
        if llm_concurrency > 1:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=llm_concurrency) as executor:
                ...
                for future in concurrent.futures.as_completed(future_to_row):
                    ...
                    total_processed = stats["succeeded"] + stats["failed"] + stats["skipped"]
                    print(
                        f"[rule+llm] {card_type} progress: batch_end={processed}/{len(target_rows)} "
                        f"global_processed={total_processed} success={stats['succeeded']} "
                        f"failed={stats['failed']} skipped={stats['skipped']} cache_hits={stats['cache_hits']}",
                        file=sys.stderr,
                    )
        ...
        for offset in range(0, len(target_rows), batch_size):
            ...
            if not provider_healthy:
                ...
                print(
                    f"[rule+llm] {card_type} progress: batch_end={offset + len(batch)}/{len(target_rows)} "
                    f"global_processed={processed} success={stats['succeeded']} failed={stats['failed']} skipped={stats['skipped']}",
                    file=sys.stderr,
                )
                continue
...
def regression_eval_rows() -> list[dict[str, Any]]:
    load_url_path = (
        "harmonyos-6.1-8k/API/ArkWeb/cj-apis-webview/ohoswebwebviewWebview/"
        "class_WebviewController/func_loadUrlTT_ArrayWebHeader_where_T_ResourceStr.md"
    )
    web_paths = [
        load_url_path,
        "harmonyos-6.1-8k/API/ArkWeb/cj-apis-webview/ohoswebwebviewWebview",
        "harmonyos-6.1-8k/API/arkui-cj/cj-web-web/Web/示例代码_c46e8890.md",
    ]
    return [
        {
        ...
        },
        {
            ...
            "expected_paths": [
                "harmonyos-6.1-8k/Guide/web/cj-web-page-loading-with-web-components/使用Web组件加载页面/加载本地页面/加载本地页面_1.md",
                "harmonyos-6.1-8k/API/arkui-cj/cj-web-web/Web/示例代码_c46e8890.md",
            ],
            "category": "natural",
            "card_type": "regression",
...
def assert_default_llm_publishable(
    ...
    incomplete = [
        f"{card_type}: {llm_stats['by_card_type'][card_type]['succeeded']}/{expected[card_type]}"
        for card_type in llm_card_types
        if llm_stats["by_card_type"][card_type]["succeeded"] != expected[card_type]
    ]
    if llm_stats.get("failed", 0) or incomplete:
        raise RuntimeError(
            "rule+llm 默认索引发布门禁失败，未写入 index/: "
            + "; ".join(incomplete or [f"failed={llm_stats.get('failed')}"])
        )
...
def build(
    ...
    if mode == "rule+llm":
        ...
        aliases = build_aliases(tasks, apis)
        assert_default_llm_publishable(index_dir, llm_card_types, tasks, apis, examples, doc_cards, llm_stats)
