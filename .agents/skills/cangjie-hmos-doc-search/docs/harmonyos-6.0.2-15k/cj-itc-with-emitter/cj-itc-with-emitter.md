...
2. 订阅事件。
    ...
    let callback1 = EventCallback(
        "callback1",
        {
            data: EventData => AppLog.info("11111111111111111")
        }
    )
...
3. 发送事件。
    ...
    let event = InnerEvent(1, priority: LOW)
    ...
    //订阅该事件，并接收eventData数据。
        ...
        "callback",
        {
            data: EventData =>
            let map = data.data
            AppLog.info("size = ${map.size}")
            for (i in 0..map.size) {
                match (map
                    .get("${i}")
                    .getOrThrow()) {
                    case INT64(v) => AppLog.info("Int64 value = ${v}")
                    case BOOL(v) => AppLog.info("Bool value = ${v}")
                    case STRING(v) => AppLog.info("String value = ${v}")
    ...
    p.add("content", EventDataType.STRING("emitter"))
    p.add("id", INT64(1))
    p.add("isEmpty", BOOL(false))

    // 发送eventId为1的事件，事件内容为eventData。
    Emitter.emit(event, data: EventData(p))
    ```
...
4. 取消事件订阅。
    ...
    > **说明：**
    >
    > 当不需要订阅某个事件时，需要及时取消订阅避免造成内存泄漏。取消事件订阅使用off接口进行取消，设置要取消的事件ID。
