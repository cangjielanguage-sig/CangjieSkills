...
HIGH_VALUE_TASKS = [
    ...
    {
        "task_id": "notification.send",
        "title": "发送通知",
        "aliases": ["通知", "Notification", "推送通知", "本地通知"],
        "intent": "发送系统通知，包括基础文本通知、带图片通知、进度条通知等。",
        "when_to_use": ["应用需要向用户推送消息", "下载进度展示", "后台任务完成通知"],
        "recommended_apis": ["arkui.notification"],
        "optional_apis": [],
        "path_keywords": ["cj-notification", "NotificationManager", "publish", "NotificationRequest", "通知"],
        "example_keywords": ["notification", "通知"],
        "tags": ["notification", "background"],
    },
    {
        "task_id": "database.query",
        "title": "数据库查询",
        "aliases": ["数据库查询", "SQL查询", "RdbStore查询", "RelationalStore"],
        "intent": "使用关系型数据库 RdbStore 进行数据增删改查和谓词查询。",
        "when_to_use": ["需要本地持久化结构化数据", "需要执行 SQL 查询"],
        "recommended_apis": ["data.relational-store"],
        "optional_apis": [],
        "path_keywords": ["cj-apis-relational_store", "RdbStore", "ResultSet", "querySql", "insert", "delete"],
        "example_keywords": ["relational_store", "RdbStore"],
        "tags": ["database", "data"],
    },
    {
        "task_id": "web.load",
        "title": "加载网页",
        "aliases": ["Web组件", "加载网页", "WebView", "嵌入H5页面"],
        "intent": "使用 Web 组件加载和显示网页内容，支持本地 HTML 和远程 URL。",
        "when_to_use": ["在应用中嵌入网页", "加载 H5 页面", "展示在线内容"],
        "recommended_apis": ["web.embed.basic"],
        "optional_apis": [],
        "path_keywords": ["cj-web", "WebView", "loadUrl", "Web组件", "加载网页"],
        "example_keywords": ["Web", "loadUrl"],
        "tags": ["web", "ui"],
    },
    {
        "task_id": "theme.dark",
        "title": "深色模式适配",
        "aliases": ["深色模式", "Dark Mode", "主题切换", "颜色模式"],
        "intent": "适配应用界面的深色模式，响应系统颜色模式变化。",
        "when_to_use": ["用户开启深色模式", "需要手动切换主题", "适配系统外观"],
        "recommended_apis": ["arkui.environment"],
        "optional_apis": [],
        "path_keywords": ["darkMode", "深色模式", "colorMode", "Environment", "ForceDark"],
        "example_keywords": ["darkMode", "深色"],
        "tags": ["theme", "ui", "accessibility"],
    },
    {
        "task_id": "background.schedule",
        "title": "后台任务调度",
        "aliases": ["后台任务", "WorkScheduler", "延迟任务", "后台执行"],
        "intent": "在应用退到后台时执行短时任务、长时任务或延迟任务。",
        "when_to_use": ["需要在后台执行任务", "需要定时或周期性任务", "应用退后台后需要保活"],
        "recommended_apis": [],
        "optional_apis": [],
        "path_keywords": ["cj-backgroundtasks", "backgroundTask", "WorkScheduler", "continuousTask", "后台任务"],
        "example_keywords": ["backgroundTask", "后台"],
        "tags": ["background", "scheduling"],
    },
    {
        "task_id": "permission.general",
        "title": "权限申请",
        "aliases": ["权限申请", "动态权限", "用户授权", "敏感权限"],
        "intent": "在运行时向用户申请敏感权限，检查权限状态。",
        "when_to_use": ["需要访问相机/位置/存储等敏感数据", "权限被拒绝需要引导用户"],
        "recommended_apis": ["arkui.permission"],
        "optional_apis": [],
        "path_keywords": ["requestPermissionsFromUser", "AtManager", "grantStatus", "向用户申请授权"],
        "example_keywords": ["permission", "权限"],
        "tags": ["security", "permission"],
    },
    {
        "task_id": "state.persist_limit",
        "title": "持久化数据限制",
        "aliases": ["PersistentStorage 限制", "持久化2kb限制", "状态持久化"],
        "intent": "了解 PersistentStorage 的数据大小限制及超出限制的处理方案。",
        "when_to_use": ["持久化数据超过2KB", "需要存储大量状态数据"],
        "recommended_apis": ["state.persiststorage"],
        "optional_apis": [],
        "path_keywords": ["PersistentStorage", "persistProp", "2kb", "preferences"],
        "example_keywords": ["PersistentStorage", "持久化"],
        "tags": ["state", "storage"],
    },
    {
        "task_id": "ability.lifecycle_flow",
        "title": "Ability 生命周期流程",
        "aliases": ["Ability 生命周期", "UIAbility 生命周期", "前后台切换"],
        "intent": "了解 UIAbility 从创建到销毁的完整生命周期及回调时机。",
        "when_to_use": ["需要了解 Ability 生命周期", "前后台切换处理", "资源初始化与释放"],
        "recommended_apis": ["arkui.ability"],
        "optional_apis": [],
        "path_keywords": ["cj-uiability-lifecycle", "AbilityLifecycleCallback", "onCreate", "onDestroy", "onForeground", "onBackground"],
        "example_keywords": ["lifecycle", "生命周期"],
        "tags": ["ability", "lifecycle"],
    },
    {
        "task_id": "ble.communication",
        "title": "BLE 蓝牙通信",
        "aliases": ["蓝牙通信", "BLE", "低功耗蓝牙", "GATT"],
        "intent": "使用 BLE（低功耗蓝牙）进行设备发现、连接和数据通信。",
        "when_to_use": ["需要连接蓝牙外设", "需要传输低功耗传感器数据"],
        "recommended_apis": [],
        "optional_apis": [],
        "path_keywords": ["cj-connectivity", "BLE", "GATT", "Bluetooth", "蓝牙"],
        "example_keywords": ["BLE", "bluetooth"],
        "tags": ["connectivity", "bluetooth"],
    },
    {
        "task_id": "database.create",
        "title": "创建数据库",
        "aliases": ["创建数据库", "初始化数据库", "建表", "getRdbStore"],
        "intent": "在应用中创建关系型数据库并初始化表结构。",
        "when_to_use": ["应用首次启动需要建库", "需要创建新的数据表"],
        "recommended_apis": ["data.relational-store.basic"],
        "optional_apis": [],
        "path_keywords": ["getRdbStore", "StoreConfig", "create", "executeSql", "数据库创建"],
        "example_keywords": ["RdbStore", "数据库"],
        "tags": ["database", "data"],
    },
    {
        "task_id": "environment.params",
        "title": "Environment 环境参数",
        "aliases": ["Environment", "设备环境参数", "环境变量"],
        "intent": "获取设备环境参数（语言、颜色模式、屏幕密度等）并响应变化。",
        "when_to_use": ["需要获取系统环境参数", "根据设备属性调整UI"],
        "recommended_apis": ["arkui.environment"],
        "optional_apis": [],
        "path_keywords": ["cj-environment", "Environment", "envProp", "设备环境"],
        "example_keywords": ["Environment", "环境"],
