...
- 当装饰的对象是数组时，无法单独感知某个数组项的变化，但能感知整体的变化。如果要感知内部的变化，需要使用[ObservedArray\<T\>](../../cj-state-rendering-componentstatemanagement/.overview.md)和[ObservedArrayList\<T\>](../../cj-state-rendering-componentstatemanagement/.overview.md)。示例请参见[数组类型的@Link](#数组类型的link)。
