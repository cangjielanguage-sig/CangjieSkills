...
| 规则/任务 | 摘要 |
|---|---|
| [1. 基本用法](1-基本用法.md) | 新模块优先由 `cjpm init --name <合法包名> --type=executable` 生成完整清单和源码骨架，不要手写不完整的 `cjpm.toml`；程序参数用 `cjpm run --run-args="..."` 传递。 |
| [2. 项目结构与 cjpm.toml](2-项目结构与-cjpm-toml/index.md) | 注意：`[package]` 与 `[workspace]` 互斥，不可同时使用。 |
| [3. 常用命令选项表](3-常用命令选项表/index.md) | `run-args` 是 1.0.5 的参数透传入口，但会拆分实参内的 `=`，且 Windows 上不透传程序非零状态；子页另列 build/test、锁文件、依赖树和安装选项。 |
