...
![效果示例图](./figures/process-start-intent-panel.png)
...
通过mailto电子邮件协议，可以创建指向电子邮件地址的超链接，方便用户通过网页或应用中的超链接直接跳转电子邮件应用。详情请参见[拉起邮件类应用（mailto方式）](cj-start-email-apps-by-mailto.md)。

## 通过startAbility接口打开文件

开发者可以通过调用startAbility接口，由系统从已安装的应用中寻找符合要求的应用，打开特定类型的文件。详情请参见[拉起文件处理类应用](cj-file-processing-apps-startup.md)。
