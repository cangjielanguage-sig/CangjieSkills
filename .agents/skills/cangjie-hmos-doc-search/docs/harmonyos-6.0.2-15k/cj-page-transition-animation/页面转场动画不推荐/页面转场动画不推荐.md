...
两个页面间发生跳转，一个页面消失，另一个页面出现，这时可以配置各自页面的页面转场参数实现自定义的页面转场效果。[页面转场](../../cj-animation-pagetransition/.overview.md)效果写在pageTransition函数中，通过[PageTransitionEnter](../../cj-animation-pagetransition/.overview.md)和[PageTransitionExit](../../cj-animation-pagetransition/.overview.md)指定页面进入和退出的动画效果。
