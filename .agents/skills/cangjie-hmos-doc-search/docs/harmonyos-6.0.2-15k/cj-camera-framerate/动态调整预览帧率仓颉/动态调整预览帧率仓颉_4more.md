...
1. 调用[createSession](../../cj-apis-multimedia-camera/.overview.md)创建会话（Session）时，需要指定模式为NORMAL_PHOTO或NORMAL_VIDEO。仅当Session处于NORMAL_PHOTO或NORMAL_VIDEO模式时，支持调整预览流帧率。调整帧率的创建会话方式见[创建Session会话并指定模式](#创建session会话并指定模式)。
...
相机使用预览等功能前，均需创建相机会话，调用[CameraManager](../../cj-apis-multimedia-camera/.overview.md)创建一个会话。

创建会话时需指定[SceneMode](../../cj-apis-multimedia-camera/.overview.md)为NORMAL_PHOTO或NORMAL_VIDEO，创建出的Session处于拍照或录像模式。
