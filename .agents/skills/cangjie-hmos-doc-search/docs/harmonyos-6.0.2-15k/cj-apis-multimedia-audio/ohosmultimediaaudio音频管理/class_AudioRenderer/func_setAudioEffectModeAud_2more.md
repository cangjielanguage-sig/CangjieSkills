...
|参数名|类型|必填|默认值|说明|
|:---|:---|:---|:---|:---|
|mode|[AudioEffectMode](#enum-audioeffectmode)|是|-|音效模式。|
...
try {
    ...
    try {
        ...
        Hilog.error(0, "setAudioEffectMode", "errCode: ${e.code}, errMessage: ${e.message}")
    }
} catch (e: BusinessException) {
...
```cangjie
public func setChannelBlendMode(mode: ChannelBlendMode)
```
...
|参数名|类型|必填|默认值|说明|
|:---|:---|:---|:---|:---|
|mode|[ChannelBlendMode](#enum-channelblendmode)|是|-|声道混合模式类型。|
...
try {
    ...
    let audioRenderer = createAudioRenderer(options)
    try {
        audioRenderer.setChannelBlendMode(ChannelBlendMode.MODE_DEFAULT)
    } catch (e: BusinessException) {
        Hilog.error(0, "setChannelBlendMode", "errCode: ${e.code}, errMessage: ${e.message}")
    }
} catch (e: BusinessException) {
    Hilog.error(0, "createAudioRenderer", "errCode: ${e.code}, errMessage: ${e.message}")
}
