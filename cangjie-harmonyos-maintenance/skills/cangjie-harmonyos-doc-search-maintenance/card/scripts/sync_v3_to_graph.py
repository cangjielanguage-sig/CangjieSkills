...
用法：
    ...
    python sync_v3_to_graph.py --validate \\
        --graph ../cangjie-harmonyos-doc-search/doc-graph/data/merged/graph.json
"""
...
def _normalize_id(raw: str) -> str:
    ...
    将非字母数字字符替换为下划线，去首尾下划线后转小写，
    确保 V3 种子节点 id 与图谱节点 id 可精确匹配。"""
...
def build_seeds(index_dir: Path) -> dict[str, Any]:
    ...
    遍历 apis.jsonl 和 tasks.jsonl，将每条卡片转为图谱节点定义：
    - API 节点: layer=2, 包含 aliases/related_apis 边
    - Task 节点: layer=1, 包含 recommended_apis/optional_apis 边
    边的 origin="v3_seed" 标记来源，供图谱构建时区分种子边和 LLM 抽取边。
    """
...
def validate_against_graph(seeds: dict[str, Any], graph_path: Path) -> dict[str, Any]:
    ...
    三级匹配策略：精确 id 匹配 → label 匹配 → 缺失。
    缺失比例超过 30% 时校验退出码为 1（在 main 中判定）。"""
