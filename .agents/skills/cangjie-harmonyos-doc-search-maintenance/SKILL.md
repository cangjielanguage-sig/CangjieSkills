...
`scripts/run_maintenance.py` 支持 `--fusion-ab-gate`：在发布候选 `index/` 前执行 `sync_v3_to_graph` + `run_ab_eval`（`real_session` / `paraphrase` / `composition`），要求 **fusion 召回不低于 V3 与 graphify**。OpenViking 对照默认关闭，需显式 `--openviking`。

## rule+llm 构建结束后的固定收尾

长时间 `build_index_v3.py --mode rule+llm` 跑完后，执行 **`scripts/run_after_rule_llm_build.sh`**：会等待构建进程退出，再导出 V3 种子、（若存在合并图）做一次 `sync_v3_to_graph --validate`，最后跑 fusion AB 并写入 `records/ab-15k-after-rule-llm.json`。
...
- `../cangjie-harmonyos-doc-search-maintenance/scripts/validate_eval_set.py` 出现 `missing_path` 时发布必须 blocked。
- 候选集失败不阻塞发布，但必须记录。
