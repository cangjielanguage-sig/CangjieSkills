...
功能:
    ...
    3. 截屏 + dump 控件树
    4. 解析控件树输出结构化摘要
    5. [新增] 可配置交互场景：点击/输入/滑动/返回/等待
    6. [新增] 交互前后二次采集，生成差异对比与断言报告

原理:
    本脚本不依赖 Hypium 测试框架，而是直接调用 hdc 命令行工具完成采集与交互。
    hdc (HarmonyOS Device Connector) 是鸿蒙的设备调试桥，类似 Android 的 adb。
    - snapshot_display / uitest screenCap: 截屏
    - uitest dumpLayout: 控件树 dump
    - uitest uiInput click x y: 模拟点击
    - uitest uiInput dblClick x y: 模拟双击
    - uitest uiInput longClick x y: 模拟长按
    - uitest uiInput swipe x1 y1 x2 y2 [speed]: 模拟滑动
    - uitest uiInput fling x1 y1 x2 y2 [stepLen] [speed]: 模拟快速滑动
    - uitest uiInput inputText x y text: 模拟文本输入（在坐标处输入）
    - uitest uiInput keyEvent keyCode: 模拟按键事件
    - aa start: 启动指定 Ability
    脚本将这些原子命令串联为: 连接 → 启动 → 基线采集 → 交互 → 二次采集 → 差异分析。
"""
...
def install_hap(hap_path: str):
    ...
    if hap_path and os.path.isfile(hap_path):
        ...
        out = run(f'hdc install -r "{hap_path}"')
        print(f"  {out}")
...
def capture_screenshot(out_dir: str) -> str:
    """截取设备屏幕并拉取到本地。尝试多种截图方式以兼容不同设备。"""
    local_path = os.path.join(out_dir, "screenshot.png")
    device_path = "/data/local/tmp/_ui_capture_screen.png"

    # 方式1: snapshot_display（物理设备常用）
    out = run(f"hdc shell snapshot_display -f {device_path}")
    if "error" not in out.lower():
        run(f'hdc file recv {device_path} "{local_path}"')
        run(f"hdc shell rm -f {device_path}")
        if os.path.isfile(local_path) and os.path.getsize(local_path) > 0:
            print(f"[OK] 截图已保存: {local_path}")
            return local_path

    # 方式2: uitest screenCap（模拟器更可靠）
    out = run(f"hdc shell uitest screenCap -p {device_path}")
    run(f'hdc file recv {device_path} "{local_path}"')
    run(f"hdc shell rm -f {device_path}")
    if os.path.isfile(local_path) and os.path.getsize(local_path) > 0:
        print(f"[OK] 截图已保存（uitest screenCap）: {local_path}")
        return local_path

    # 方式3: hdc shell screencap
    out = run(f"hdc shell screencap -p {device_path}")
    run(f'hdc file recv {device_path} "{local_path}"')
    run(f"hdc shell rm -f {device_path}")
    if os.path.isfile(local_path) and os.path.getsize(local_path) > 0:
        print(f"[OK] 截图已保存（screencap）: {local_path}")
        return local_path
...
def detect_project_info(project_dir: str) -> tuple:
    ...
    返回 (bundle_name, ability_name)，检测失败则返回 (None, None)。
    """
    bundle_name = None
    ability_name = None
...
def summarize_layout(layout_path: str, out_dir: str, bundle: Optional[str] = None) -> str:
    ...
    def parse_bounds(bounds_str):
        """解析 bounds 字符串，如 '[0,0][1080,2340]' → (x1, y1, x2, y2)。"""
    ...
    def walk(node, depth=0):
        # hdc dumpLayout 格式: {"attributes": {...}, "children": [...]}
        ...
        for child in children:
            ...
            if not child_attrs and "type" in child:
                child_attrs = child
            cb = parse_bounds(child_attrs.get("bounds", ""))
            if cb:
...
# ============================================================
# 交互引擎：解析控件树定位目标 + 执行 hdc 交互命令
# ============================================================

def _flatten_tree(node: dict) -> List[dict]:
    """将嵌套控件树扁平化为节点列表，每个节点带 attributes。"""
    nodes = []
    attrs = node.get("attributes", {})
    if not attrs and "type" in node:
        attrs = node
...
def execute_step(step: dict, flat_nodes: List[dict], screen_w: int, screen_h: int,
    ...
    返回: {"action": str, "success": bool, "detail": str, "snapshot": str|None}
    """
    ...
    if action in ("click", "long_click", "double_click", "input"):
        ...
        if action == "click":
            ...
            time.sleep(0.5)
            cmd = f'hdc shell uitest uiInput inputText "{text}"'
...
def diff_layouts(before_path: str, after_path: str) -> dict:
    ...
    before_nodes = safe_load(before_path)
    after_nodes = safe_load(after_path)
...
def evaluate_assertions(assertions: List[dict], before_path: str, after_path: str,
    ...
    after_nodes = safe_load_flat(after_path)
    before_nodes = safe_load_flat(before_path)
