<!-- cj-doc kind="api-member" level="6" id="std.overflow.interface.carryingop.carryingneg" parent="std.overflow.interface.carryingop" -->
# CarryingOp<T>.carryingNeg
...
返回一个元组，元组的第一个元素表示负号运算是否发生了截断，发生截断时为 `true`，元组的第二个元素是运算的结果。
