...
在启动目标应用组件时，会通过显式[Want](../../cj-apis-ability/.overview.md)或者隐式[Want](../../cj-apis-ability/.overview.md)进行目标应用组件的匹配。本章所述的匹配规则是：调用方传入的[Want](../../cj-apis-ability/.overview.md)参数中设置的参数如何与目标应用组件声明的配置文件进行匹配。

## 显式Want匹配原理

显式[Want](../../cj-apis-ability/.overview.md)匹配原理如下表所示。
