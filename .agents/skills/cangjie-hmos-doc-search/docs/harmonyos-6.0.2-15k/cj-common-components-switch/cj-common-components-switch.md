...
Toggle组件提供状态按钮样式、勾选框样式和开关样式，一般用于两种状态之间的切换。具体用法请参见[Toggle](../cj-button-picker-toggle/.overview.md)。
...
除支持[通用事件](../cj-universal-event-click/.overview.md)外，Toggle还用于选中和取消选中后触发某些操作，可以绑定onChange事件来响应操作后的自定义行为。
...
class EntryView {
    func build() {
        Column() {
            ...
            Row() {
                Text("Bluetooth").height(50).padding(left: 10).fontSize(16).textAlign(TextAlign.Start).backgroundColor(
                    0xFFFFFF)
                Toggle(ToggleType.SwitchType).margin(left: 200, right: 10).onChange {
                    isOn => if (isOn) {
        ...
        }.padding(10).backgroundColor(0xDCDCDC).width(100.percent).height(100.percent)
    }
}
```
