...
1. 声明文件打开能力。
    ...
    支持打开文件的应用需要在[module.json5](../../../module-configuration-file/module-configuration-file.md)配置文件中声明文件打开能力。其中uris字段表示接收URI的类型，其中scheme固定为file。type字段表示支持打开的文件类型（参见[UTD类型](../../../cj-uniform-data-type-descriptors/.overview.md)（推荐）或[MIME type类型](https://www.iana.org/assignments/media-types/media-types.xhtml?utm_source=ld246.com)），如下举例中类型为txt文件。
...
2. 应用处理待打开文件。
    ...
    声明了文件打开的应用在被拉起后，获取传入的[Want](../../../cj-apis-ability/.overview.md)参数信息，从中获取待打开文件的URI，在打开文件并获取对应的file对象后，可对文件进行读写操作。
