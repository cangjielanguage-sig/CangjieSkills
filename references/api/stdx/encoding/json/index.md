...
| 声明 | 功能 |
|---|---|
| [`JsonArray <: JsonValue`](classes/jsonarray/index.md) | JSON 数组容器；1.0.5.1 未实现 `Iterable`，遍历时使用 `getItems()`，或按 `0..size()` 生成索引并用 `operator []` 取值。 |
...
| [`JsonObject <: JsonValue`](classes/jsonobject/index.md) | JSON 对象容器；缺键读取优先用 `get` 取得 Option，`operator []` 在键不存在时抛 `JsonException`；1.0.5.1 文档未承诺 `toString()` 的键顺序。 |
...
| 声明 | 功能 |
|---|---|
| [`JsonKind`](enums/jsonkind/index.md) | `JsonKind` 标识 JsonValue 的七种形态；1.0.5.1 未实现 Equatable，须用 `match` 判别而不能用 `==`/`!=`。 |
